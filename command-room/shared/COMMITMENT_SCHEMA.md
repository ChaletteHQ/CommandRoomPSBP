...
| `review_reason` | when `pending_review` is true | One-line plain-English explanation of why review is needed (e.g., "Owner is external person — no entity record yet"). Used by the needs-your-call queue and the Commitments surfaces to compose review prompts (the retired Pulse chat's CRU-review section was the original consumer). |
