...
1. **Resolve the backend in ONE call.** `tool_discovery.discover_mail_draft_tool(tools, declared=connector_config.declared_backend("email"))` — and the `send` / `reply` / `thread_fetch` seams the same way. The seam resolves the declared backend first and refuses to substitute another product's tool; when nothing is declared it falls back to fingerprint discovery, which IS today's behavior (R4). Do **not** hand-roll the old two-step (`discover_for_category` then a bare `discover_mail_*(tools)`): every copy of it was a place the `declared=` argument could go missing, and a mail seam that silently binds the second connector reads as an empty inbox, not as an error (MAILSEAM2). NEVER name a provider tool (`create_draft`, `send_draft`) directly in skill prose (Rule 21); the per-provider mapping lives in `connector_adapters/mail.py`.
