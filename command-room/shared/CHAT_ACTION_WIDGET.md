...
| `keep as one` | Keep as one | CLUSTER1 cluster lines only (needs-your-call / staff-meeting fold / commitment-triage / backlog-sweep — NEVER held-review, its DD-4 fence). The row embeds `data.id` + `data.folded_ids` verbatim; dispatch is `commitment_cluster.apply_cluster_merge` — `clear_review_flags` on a pending survivor + one `supersede_commitment(user_confirmed=True)` per folded id, all in ONE `clu_` brain batch so a single `undo` reverses the gesture. |
...
**The SET ASIDE rows (OBSERVED1, 2026-08-24) are a second row class on this same src** — live observed-tier items (`commitment_observed`), rendered as one trailing section by `build_queue_data_view` and as the SET ASIDE section of `show watching` (`watch_gate.build_watching_view`, same src). Their `n` is the observed row's own id — `obs_<sha12>`, or the `commitment_seq_N` fallback for a legacy id-less row — and they carry the tier's OWN two-verb list, `capture_gate.OBSERVED_ROW_ACTIONS` (`confirm` / `drop`): no `already done` (nothing tracked = nothing attestable — `done_items` refuses one by name) and no `not mine` button (the tier already is "not mine"). Dispatch is UNCHANGED — the same `confirm_items` / `drop_items` calls route these ids through the observed transition (`capture_gate.promote_observed`, then `clear_review_flags` or `close_commitment(resolution="dropped")`): confirm on a set-aside row OPENS an ordinary commitment rather than clearing a flag, and the per-row result carries `promoted_id` naming what now exists. The staff-meeting fold does NOT render this row class.
