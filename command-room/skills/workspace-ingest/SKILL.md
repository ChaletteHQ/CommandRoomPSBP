...
surfaces: both
description: "Underlying two-layer ingest pipeline: extract context (people, projects, decisions, memories) from a source AND optionally copy its documents into workspace project folders. Fires on: 'ingest this folder' / 'ingest folder [path]', 'ingest [source] into my workspace', 'bootstrap my workspace from [source]'. Preview-and-confirm before any write; AUGMENT mode on existing data (dedupe makes re-runs no-ops); undo is snapshot-based and removes only what its own log recorded as created. Routes: pure context extraction lands in ingest-context; pure file filing in file-documents; this skill runs the combined pipeline. Does NOT fire on 'drop everything that import captured' (commitment-backlog-sweep — commitment rows, not entities and files), 'scan my desktop' / 'sort my downloads' alone (file-documents). Mode table, undo contract, and safety rails: Routing section in the body."
