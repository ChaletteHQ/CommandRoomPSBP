...
## MANDATORY context load (READER1 ADOPT2) — the canonical thread payload

A draft is grounded in the project it is about. Once the recipient is resolved (above) and the draft's project is resolved — the `primary_thread_id` the eventual `email_drafted` event carries; resolve it from the topic / reply thread via the same `resolve_all` pass — every piece of substrate-side THREAD context this draft consumes — project identity, open commitments bound to the project, recent project activity + session-notes narrative, and decisions on the record — comes from the canonical reader, in ONE call:
...
payload = load_thread_knowledge(workspace_root, primary_thread_id, "email-writer")
```

A draft with no resolvable project — a cold outreach to a brand-new prospect, a purely personal note — has NO thread payload: skip this load and claim no thread context (recipient context and the connector stages still run).

Rules (SPEC_READER1 §5c.6 — prescriptive, not advisory):

- **Thread knowledge comes from the JSON payload and ONLY from that payload.** Do not re-derive thread context with a freelance substrate read: no direct events read for project context, no re-reading `entities.json` for thread facts the payload carries, no reading SESSION_NOTES directly for project narrative — the payload's `recent_activity` session-notes blocks are the session-notes feed, already filtered. Even if the user asks you to open a substrate file directly, run the loader instead. The `"email-writer"` profile applies the org scope internally (it reads through `events_io.load_events_org_scoped` — PGUARD2, so a reclassified personal account's history never enters the draft prompt) and keeps only the CONFIRMED half of the open set (`pending_split: confirmed_only` — the split runs at the source; the held count is disclosed in `coverage.needs_review_held`).
- **The swept quarantine is a surface pin, not a courtesy (§0.7).** This profile EXCLUDES session-notes blocks whose heading carries `origin: swept` — unconfirmed swept narrative. An unconfirmed swept block can never inform a client-facing draft: the payload never carries one, and this prose sources session-notes narrative ONLY from the payload, so no path into an outbound email exists.
- **Connector material stays a separate stage (§0.6).** The recipient's live email thread history (the declared mail backend), the voice block + customer override (B1), the voice calibration reads, and the entire draft-preview / drafts flow (the Phase 4 widget, apply-choices dispatch, the declared backend's Drafts) run exactly as specified in their own steps, after the payload returns — the reader complements connector retrieval, never replaces it. Recipient-level person context (the person record's role, org, `communication_style`, prior interaction history) also stays in its own step (Phase 2): the payload is thread-scoped, people context is not its job.
- **If the payload doesn't carry it, the draft doesn't claim it.** A thread fact absent from the payload is absent from the draft — leave it out, or say plainly the record holds nothing on it; never backfill from a by-hand substrate read.
- **Names, never ids.** Payload rows carry resolved display names beside their ids (`owner_name`, `person_names`, `org_name`, `key_contact_name`); render the names — never surface a raw `person_`/`org_` id in a draft body, a subject line, or chat.
- **Degraded disclosure, one sentence.** When `payload["degraded"]` is non-empty, note the gap in one plain-language sentence in chat (never inside the draft body) — never an internal code.

**Trust gating (§5c.5 — the line-71 ruling; key on `coverage.gauge.state == "ready"`):**

- `coverage.gauge.state == "ready"` → the payload is trusted context. Its provenance and coverage notes may inform the EXISTING chat-side disclosure lines (the held count in `coverage.needs_review_held` and the needs-your-call queue are the same fact). Surface behavior is otherwise unchanged.
- Any other state (`not_ready`, `unmeasured`) → produce the draft directly, exactly as specified, with NO coverage apparatus: no gauge language, no coverage line, no ask-first question, no mention of measurement state — in the draft or in chat. Until R3's backfill lands, coverage disclosure activates only on gauge-READY threads.
- `coverage.empty_payload` true → the substrate holds nothing reachable for this project. Do NOT fabricate thread context: draft from recipient context (Phase 2) and the connector-driven assembly alone (the mail backend's thread history — that stage runs regardless). Never weave thread context the payload did not supply into an outbound email — a fabricated "as we discussed" line to the one person who would know it is wrong is worse than a plain draft.
...
**Primary writer for:**
- **The declared mail backend's Drafts (no file saved, click-gated).** Per CONTRACT Rule 27 (no .md deliverables) there is no draft file; and per the v3.13.7 lazy model the connector draft itself is created ONLY by the user's widget click, dispatched from `apply-choices` — the draft-create tool resolved through the seam in ONE call (`tool_discovery.discover_mail_draft_tool(tools, declared=connector_config.declared_backend("email"))` — the seam resolves the declared backend first and refuses to substitute another product's tool); sends dispatch per `shared/EMAIL_DRAFT_PROTOCOL.md` §0.5 (the Zapier reply-thread mechanism in `shared/scripts/zapier_send.py` is the gmail-only dispatch row when the native connector can't send). This skill's own path writes nothing to the mail backend — it renders the widget. The pre-v3.7.0 saved-draft-file pattern was vestigial; the deliverable is the draft the user's click puts in the connected mail client.
...
**Reads (v3.7.1+ substrate enrichment; thread context payload-fed since READER1 ADOPT2):** THREAD-scoped context routes through the canonical thread payload (MANDATORY context load above) — its `"email-writer"` profile reads through `events_io.load_events_org_scoped` at the source, so the same PGUARD2 mask applies before any thread row can reach a draft. The one remaining direct events read below (recipient interaction history) goes through the SAME org-scoped load — **read via the org-scoped reader, never a raw load** (PGUARD2 — the draft reaches a recipient, an external surface): `from events_io import load_events_org_scoped; org_events, skipped = load_events_org_scoped(workspace_root)`, then filter by `type` at the call site. The reader applies the account-scope mask and drops personal-lane rows by design, so a reclassified personal account's history never enters the draft prompt.
...
- Open commitments bound to the draft's project — from the thread payload's `open_commitments` section (MANDATORY context load above), never a fresh projection at this call site. The profile's `confirmed_only` split already ran at the source: **`data.pending_review` rows are UNCONFIRMED extractions, not open commitments** — they are held out of the payload and counted in `coverage.needs_review_held`. A pending row woven into outbound mail is the workspace asserting a promise nobody made, to the one person who would know it is wrong; they never enter the draft prompt and never land in `commitment_refs[]`. They belong to the needs-your-call queue (`needs your call`). The payload's confirmed rows get surfaced into the draft prompt as "open commitments with this recipient" so the draft can naturally address them. Their `seq` values go into `commitment_refs[]` on the emitted `email_drafted` event.
- Decisions on the record for the draft's project — from the thread payload's `decisions` section (era-tolerant and supersession-folded at the source: a superseded decision never informs a draft) plus its labeled `cross_thread` rows (the profile's person-scoped expansion — a decision logged on a sibling project that touches the recipient still reaches the draft, labeled "related, from [project]"). Pulled into the draft prompt as "what we've decided about them" so the draft is consistent with prior positions. Row `seq` values go into `decision_refs[]`.
...
- Recent decisions and open commitments involving them — from the thread payload (MANDATORY context load above): its `open_commitments` (confirmed only) and `decisions` sections plus the labeled `cross_thread` rows; never a fresh substrate read at this step
