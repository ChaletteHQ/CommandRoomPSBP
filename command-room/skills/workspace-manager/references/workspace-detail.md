...
**Active Commitments:** For new commitments: add with commitment text, due date, source (which meeting/email), status "Open." For delivered commitments: update status to "Delivered" with today's date. For overdue commitments: update status to "Overdue" (automatic — any commitment past due date). Never remove commitments — they compress to Commitment History per maintenance Rule 3.
