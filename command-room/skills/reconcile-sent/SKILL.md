---
name: reconcile-sent
description: "Silent, single-purpose maintenance task — closes commitments the CEO completed by sending a follow-up DIRECTLY from Gmail (outside the product's draft->send path). Fires as a scheduled task on weekday mornings (6:45 AM, before the morning brief) and can be run manually with 'reconcile my sent mail' / 'reconcile sent'. A wide one-time catch-up runs via 'catch up my sent mail' / 'reconcile the last N days' / 'reconcile my backlog' — fetches a 30-day window regardless of the cursor to clear backlog stranded behind a stale cursor (Bug #101). Its ONLY job is: fetch Gmail Sent (since the cursor, or a 30-day window on a first run / catch-up), match against open commitments, write the closures + advance the cursor, and self-validate that the work actually landed. It does NOT produce a reader-facing brief — the morning brief reads what this task wrote. This single-responsibility split exists because reconciliation is an invisible substrate write that loses to visible deliverables when co-located with them (Bug #98-v3: three folds into the brief/inbox were all skipped; the brief diagnosed the structural cause itself and recommended moving it out)."
---

# Reconcile Sent — silent commitment reconciliation

This task has exactly one job and nothing to hide behind. Reconciliation is a
write-side maintenance job (cursor semantics + idempotent closures); the morning
brief is a read-mostly render. They were co-located and the invisible write lost
every time (Bug #98-v3). Here it IS the job, so there is nothing to deprioritize
it against — the same reason the silent Sunday `cleanup` task runs reliably.
