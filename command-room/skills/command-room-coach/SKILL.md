...
- `prospect_conversion_candidates` — from `shared/scripts/prospect_conversion_detector.py::detect_prospect_conversion_candidates(workspace_root)`. Prospects that look like they've become clients — a paid or signed fact: a won deal thread, a `deal_won` / invoice / payment / agreement event, or signing language in recent events — but are still registered `relationship_type: prospect`. DEALNAG1 + M's ruling 4 (2026-09-03): a paid or signed fact promotes the org automatically (`org_promotion` — receipt, one CHANGED line, undo), so it is never a nudge here; an active client engagement edge or an active project alone is NOT a signal at all (the `new prospect` command writes exactly that shape for the sales conversation itself). What remains is the ambiguous lane the detector returns — render that, nothing else. **Detect-and-nudge, NEVER auto-flip (Bug #92):** when there are candidates, surface a nudge — *"[Name] looks like a client now ([reason]) — say `[Name] is now a client` to convert."* The CEO confirms; the conversion runs through the Bug #91 typed-writer path. Lead with HIGH-confidence candidates. Do NOT change `relationship_type` yourself — only suggest.
