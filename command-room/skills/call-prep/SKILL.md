...
- **Thread knowledge comes from the JSON payload and ONLY from that payload.** Do not re-derive thread context with a freelance substrate read: no direct events read for project context, no re-reading `entities.json` for thread facts the payload carries, no reading SESSION_NOTES directly for the brief's narrative — the payload's `recent_activity` session-notes blocks are the session-notes feed, already filtered (this profile excludes unconfirmed `origin: swept` blocks, so an unconfirmed swept block never reaches a prep brief). Even if the user asks you to open a substrate file directly, run the loader instead.
- **Connector material stays a separate stage (§0.6).** Calendar, the declared mail backend, the declared chat backend, and Granola transcript retrieval run exactly as specified in their own steps, after the payload returns, and merge into the brief — the reader complements connector retrieval, never replaces it. Attendee-level person context (person records, `person_timeline_points`) also stays in its own steps: the payload is thread-scoped, people context is not its job.
- **If the payload doesn't carry it, the brief doesn't claim it.** A thread fact absent from the payload is absent from the brief — omit the section (omit-don't-pad) or say plainly that the record holds nothing on it; never backfill from a by-hand substrate read.
...
- **Degraded disclosure, one sentence.** When `payload["degraded"]` is non-empty, note the gap in one plain-language sentence where it affects a section — never an internal code.

**Trust gating (§5c.5 — the line-71 ruling; key on `coverage.gauge.state == "ready"`):**

- `coverage.gauge.state == "ready"` → the payload is trusted context. Its provenance and coverage notes may inform the brief's EXISTING disclosure lines (a truncation note; the held-count pointer — the payload's pointer note and block ④'s `needs your call` line are the same fact). Surface behavior is otherwise unchanged.
- Any other state (`not_ready`, `unmeasured`) → build the brief directly, exactly as specified, with NO coverage apparatus: no gauge language, no coverage line, no ask-first question, no mention of measurement state — in the document or in chat. Until R3's backfill lands, coverage disclosure activates only on gauge-READY threads.
- `coverage.empty_payload` true → the substrate holds nothing reachable for this thread. Do NOT fabricate thread context: omit the substrate-fed sections and build the brief from the connector-driven assembly alone (calendar, mail, chat, transcripts — those run regardless). Never render thread context the payload did not supply.
...
3. Pulls project context from the canonical thread payload (the MANDATORY context load above) — identity, recent project activity, and session-notes narrative — plus `PROJECT_CONTEXT` background. SESSION_NOTES are never read directly for thread narrative; the payload's blocks are the feed
4. Reads `entities.json` for relationship context on each attendee — how you know them, last interaction, key notes, open items (canonical people layer per `shared/PASSIVE_CAPTURE.md`; `_hq/PEOPLE.md` is still read as a legacy fallback if present)
5. Searches **the declared mail backend** for recent email threads with attendees — resolved in ONE call, `tool_discovery.discover_mail_search_tool(tools, declared=connector_config.declared_backend("email"))`, with `tool_discovery.discover_mail_thread_fetch_tool(tools, declared=connector_config.declared_backend("email"))` when a full thread is needed. The attendee scope is an INTENT, spelled with the adapter's own keys and compiled per provider by `connector_adapters/mail.py` — never a provider query operator, provider field, or URL host: `{"any_of": [{"from": "<attendee address>"}, {"to": "<attendee address>"}], "not_draft": true, "after": "<the last-touch date>"}`. Both directions are deliberate — mail the CEO SENT this attendee is half the delta. The adapter's vocabulary is a CLOSED set (`unread`, `in_inbox`, `in_sent`, `from_me`, `not_draft`, `from`, `to`, `subject`, `newer_than`, `after`, `message_id_lookup`, `any_of`) and a key outside it compiles to NOTHING — the search does not error, it silently widens. `since` is the near-miss to know about: it reads like a key, it appears in the adapter's own module docstring, and it is not compiled. Use `after`. `tool_id is None` means this prep has no mail: see the mail-resolution rule under Connected Tools.
6. Searches the declared chat backend for relevant conversations
7. Takes the open commitments tied to this project from the thread payload's `open_commitments` section — confirmed rows only (the profile's pointer split holds back unconfirmed extractions; its pointer note and block ④'s `needs your call` line are the same fact), each row carrying `seq` and its meeting/email `source_ref` (the profile's meeting-links flag); undated confirmed rows stay visible (F-44). (`_hq/MASTER_TRACKER.md` is still read as a legacy fallback if present)
...
- **Changed Since Last Touch (block ②; replaces the prior-brief-gated "Since Your Last Brief"):** what moved since the last meeting with these attendees — project events, reschedules, AND **overnight mail from the declared mail backend scoped to attendee addresses** (cost-bounded: one search per meeting, cap 10 threads). Present whenever there is any delta signal, prior brief or not — the F-60 dogfood brief missed an attendee's overnight deliverables because the old section only fired off a prior brief file. 3-6 bullets, each one fact with its source + date.
- **Progress Since You Last Met:** What's been done across this project since you last met (the thread payload's `recent_activity` — event rows + session-notes blocks — plus deliverables)
...
- **Decisions Already On The Record:** Decisions previously logged for this project (the thread payload's `decisions` section — era-tolerant, supersession already folded, so a superseded decision never appears here) — the "don't relitigate" block. 2-5 bullets max, each one line: decision + date. Skip if no priors exist.
- **Decisions Needed (block ③ — DECIDE):** the open decisions this meeting is positioned to close, from the `decision_pending` rows in the thread payload's `decisions` section, with the tradeoffs of each direction
- **Cross-Project Insights:** Patterns from related projects that might bear on this conversation — seed from the thread payload's `cross_thread` rows (attendee-linked notes and decisions on sibling threads, each labeled "related, from [project]")
...
- **Decisions Already On The Record** — bullet list, 2-5 items max. Pull from the thread payload's `decisions` section. Format: `<decision in one line> — <date>`. Skip if no priors exist; never pad. Purpose: stop the user from re-litigating something already settled.
...
- **Risks / Watch-outs** — at least 1 if any sensitive thread or recent friction exists. Pull from the thread payload: `cracks_watch_feedback` rows in `recent_activity`, or `decision_pending` rows in `decisions`. Omit if everything is clean.
...
- **Changed Since Last Touch** — same delta logic as external: project events, reschedules, and overnight mail from the declared mail backend scoped to attendee addresses since the last touch (cost-bounded: one search per meeting, cap 10 threads)
- **Project events since last meeting** — what's moved on the project itself in the last 14 days (the thread payload's `recent_activity` event rows, narrowed to the last 14 days — the profile's window is wider; never a by-hand events read)
...
- **The declared mail backend** — Search for recent email threads with attendees. Resolve the tools through the seams in ONE call each — `tool_discovery.discover_mail_search_tool(tools, declared=connector_config.declared_backend("email"))` and `tool_discovery.discover_mail_thread_fetch_tool(tools, declared=connector_config.declared_backend("email"))`. The seam resolves the declared backend first, using that provider's own operation vocabulary, and refuses to substitute another product's tool; with nothing declared it falls back to fingerprint discovery and names both connectors in `result.mail_ambiguous` when the workspace has two. Never name a provider tool. The attendee scope is an INTENT, spelled with the adapter's own keys and compiled per provider by `connector_adapters/mail.py` — never a provider query operator, provider field, or URL host: `{"any_of": [{"from": "<attendee address>"}, {"to": "<attendee address>"}], "not_draft": true, "after": "<the last-touch date>"}`. Both directions are deliberate — mail the CEO SENT this attendee is half the delta. The adapter's vocabulary is a CLOSED set (`unread`, `in_inbox`, `in_sent`, `from_me`, `not_draft`, `from`, `to`, `subject`, `newer_than`, `after`, `message_id_lookup`, `any_of`) and a key outside it compiles to NOTHING — the search does not error, it silently widens. `since` is the near-miss to know about: it reads like a key, it appears in the adapter's own module docstring, and it is not compiled. Use `after`. The cost bound (one search per meeting, cap 10 threads) is stated with the Changed Since Last Touch block and is not restated here. **The mail resolution has exactly three outcomes, and the prep records which one it got (SPEC PREPSEAM1 DD-3):** the seam resolved a tool and the read succeeded → `sources.mail: "read"`; **no** mail backend resolves at all (`tool_id is None`, nothing declared and nothing discovered) → `sources.mail: "absent"`, the mail-derived material simply does not exist for this prep and the prep is NOT degraded; the declared backend is present but the resolution or the read ERRORS → `sources.mail: "failed"`, and the generator RAISES so the leg records that meeting `degraded` with the reason (`prep_leg.run_prep_leg` catches it per meeting). A resolution failure that is swallowed and rendered as a thin brief is the defect this rule exists to end — an absence and a failure look identical in the document and must never look identical on the record.
