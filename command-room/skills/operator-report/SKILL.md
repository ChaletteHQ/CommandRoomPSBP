...
- Cold relationships flagged. Count `type: pattern_break_detected` events with `source_skill ∈ {dormant-customer-scan, insight-generator, pulse}` — `pulse` stays in the set as a FOSSIL reader (LIFECYCLE1 retired the chat; its historical events are append-only and must keep counting) (the canonical dormancy-flag event type per events.schema.json — pre-v3.13.6 this spec called for `dormant_flag` which isn't in the enum and would have silently returned 0). **Back-compat (client migration):** normalize each event's `source_skill` through `source_skill_compat.normalize_source_skill` before the set check so workspaces whose Pulse history predates the v2.14.27 rename (`source_skill='cr-dont-forget'`) still count — it normalizes to `pulse`. events.jsonl is append-only; never rewrite it.
- Decisions logged from real interactions. Count `type: decision` events with `source_skill: meeting-notes` or `decision-log`.
- Aging follow-ups surfaced. Count open `type: commitment` events whose `data.due` is past the window-end date — i.e., commitments that aged into overdue status during the window — **excluding rows with `data.pending_review` true (unconfirmed extractions are not open commitments; they sit in the needs-your-call queue until the user confirms or drops them)**. (Pre-v3.13.6 this spec called for `type: aging_followup` which isn't in the enum.) The captured/inflow counts above are deliberately NOT filtered this way — they measure capture activity, not the open book.
