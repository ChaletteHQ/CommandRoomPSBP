...
- **'what's waiting on me'** → the ranked open queue as numbered lines (each item's `render_line` or evidence, verbatim — never re-decide inclusion), ending with: *"Say `staff meeting` to work through these with one-tap actions."* Explicit asks see the FULL set (deliberately exempt from the daily card's shown-marker dedup — that is why the projector call passes surface `system-health`).
...
2. **"What's waiting on you"** — the COMPLETE ranked queue via `brain_proposals.load_open_proposals(ws, "staff-meeting")` (full-set exemption from daily dedup — R2), paginated as design: pages of 10 with the canonical `show more` verb, never a size fallback.
