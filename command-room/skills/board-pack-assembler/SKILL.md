...
**§7 Appendices** — pipeline by stage, hiring slate, QB financial detail. **Pipeline-by-stage is real data as of SPEC PIPE1:** build it from open deal threads (`deal_state.list_open_deals`) + `pipeline_math` — one row per stage (lead / qualified / proposal_sent / negotiating) with count and summed stated value, plus the period's moves from `deal_stage_changed` / `deal_won` / `deal_lost` events. Untracked deal threads render as one honest count line ("N deals tracked without stages yet"), never invented stages. **Zero deals → DROP the appendix section entirely** (no hardcoded counts, no empty table — G7). Values are user-stated only; a stage with no valued deals shows count-only.
