...
- **Format selection (SPEC OUT5).** Before rendering, resolve the backend: `output_profile.resolve_format_for_kind("board_pack", workspace_root, override=...)` — pass `override="docx"` / `override="premium_html"` when the ask itself says so ("as a doc" / "as HTML" beats the profile for that render). `"docx"` (the unconfigured default) → `make_brief` exactly as above. `"premium_html"` → `shared/scripts/premium_html.py` `make_premium_brief(brief_kind="board_pack", ...)` with the SAME assembled `sections` + `exec_header` + `asks` payload (one assembly, two backends — the same gate stack runs on both, parity-pinned by G16; a `gate_ran` event with `surface: premium_html` lands the same way). Output: the same routed folder and filename with `.html`; link via `get_brief_artifact_url()`; after the call returns, CHECK the file exists on disk before linking. Never hand-compose HTML around the chokepoint — the premium template is rendered only by `make_premium_brief`.
...
**Scorecard offer (SPEC OUT7 — the lighter artifact).** If, mid-flow, the ask is really for the between-packs KPI view — "how are we tracking against targets", "just the KPIs", "a scorecard for the QBR", "I don't need the whole pack, just the numbers" — offer the **KPI scorecard** as the lighter artifact BEFORE assembling a full 6–8-page pack: a one-page KPIs-vs-targets scorecard (tile band, streaks, watch flags, trend lines) rendered by `shared/scripts/scorecard.py` `build_scorecard(readings, period="month", data_through=…)` → `make_brief(brief_kind="kpi_scorecard", …)`, or `period="quarter"` for the QBR pre-read. Same substrate, same KPI assembler as §2 — so it's a cheaper cut of the same numbers. This is a mid-flow offer only (you're already in board-pack); it is NOT a cold trigger — "KPI scorecard" as a standalone phrase does not route yet (G11-blocked, SPEC OUT7 §3d), so never tell the user it does. If they confirm they want the pack, proceed with the pack.
...
- `_hq/data/events.jsonl` — ALL events in the reporting period. **Read via the org-scoped reader, never a raw load** (PGUARD1 — this is an EXTERNAL-audience artifact, the highest-severity leak class): `from events_io import load_events_org_scoped; events, skipped = load_events_org_scoped(workspace_root)`. The reader applies the account-scope mask and drops personal-lane rows by design. Additionally, after saving the .docx run the leak scan with the board surface declared — `scan_docx_for_leaks(path, surface="board")` — so a personal fingerprint that reaches the rendered pack BLOCKS. Event types consumed:
...
- `_hq/data/events.jsonl` — `type == "decision"` events about KPI targets (e.g., "Q1 MRR target = $450K" — surfaces in the KPIs vs Targets section). Same org-scoped read as above — one `load_events_org_scoped` call serves both.
...
6. (Per `deck_companion` / explicit deck ask) Render the .pptx via `shared/scripts/deck_writer.py` `make_deck` from the SAME assembled sections (Phase 4).
...
- Org history (SPEC HIST1): `render_org_history.compile_org_history(workspace_root, org_id)` for each org the pack covers — derived stats (last touch from events, people count, meeting cadence, money tag) for §1 context, plus the compiled `facts` (org_fact_observed — recorded company context/news, e.g. a raise or a leadership change) as §1/§4 material. Recorded facts are sourced user/confirmed statements — cite them as such, never as inferred trend language.
...
**§2 KPIs vs Targets** — **build this section by calling `shared/scripts/scorecard.py` `build_kpi_section(readings)` (SPEC OUT7) — the single KPI-section assembler; board-pack §2 is its first caller.** Assemble the substrate-derived `readings` the way you always have (QuickBooks + KPI-target `decision` events + the prior pack — the KPI list inherited from the prior pack unless a `decision` event in period adjusted it; each reading is `{name, unit, current, target, higher_is_better?, history?}`, every number read from substrate, NEVER re-typed). `build_kpi_section` returns the section dict — the **KPI tile band** (SPEC OUT1 §4) ABOVE the full **table** (metric / current / target / vs-target / trend), plus the Current-vs-Target dollar bar (the §2 charts note below) — computing the vs-target arrows, the drop-empty tiles (a KPI with no current value gets NO tile; zero is a real value and renders), and the no-blank-cell em dashes for you. The scorecard skill renders the SAME section from the SAME assembler, so the pack and the standalone scorecard can never disagree on a KPI — one implementation, two consumers (the components.py move again). The tile band is the top 4–5 KPIs; each tile's `value` is the current figure, `label` is the KPI name with a target-delta arrow (`▲`/`▼`/`▬`) where a target exists.

**§2 charts (SPEC OUT3 — best-effort, under the tile band, table always kept).** `build_kpi_section` (above) ALREADY attaches the **KPI bar (`bar_grouped`)** — Current vs Target over the dollar KPIs (`scorecard.kpi_dollar_bar_spec`: one unit per chart per `shared/CHART_SELECTION.md`, skipped below 2 dollar KPIs). You may APPEND one more chart to the returned section's `charts` list — never a second bar, never a hand-composed SVG (charts.py is the one chart owner):
- **Period delta waterfall (`waterfall`)** — the period's dollar walk from the SAME `deal_won` / `deal_lost` events §3/§4 already cite: one step per event with a stated value (`label` = the deal name, `delta` = +value on a win, −value on a loss, `"unit": "$"`). Only events with user-stated values contribute (never estimated); fewer than 2 valued steps → no waterfall.
Both render through `charts.try_chart_png` inside `make_brief` automatically — a refused shape or a machine with no rasterizer drops the chart and §2 renders exactly as before (the table/tiles are the fallback, byte-identical). Never hand-compose chart markup or numbers.

**§3 Wins** — top 3 outcomes pulled from `outcome_positive` events ranked by impact. `deal_won` events in period (SPEC PIPE1) are win candidates too — "Closed [Deal] — $52K" with the stated value via quantify, never estimated.

**§4 Concerns** — top 3 pulled from `pattern_break_detected` + dormancy signals + lost-deal events + recorded org facts (SPEC HIST1: an `org_fact_observed` in period whose content is concern-shaped — a churn signal, a leadership departure, adverse company news — is real, sourced material; quote the recorded fact with its date, never embellish it). Each concern has 1-line mitigation. **Lost-deal events are real as of SPEC PIPE1:** read `deal_lost` events in period via `deal_state.load_deal_events` — each carries the org, the stated value (when the user gave one — never estimated), and the loss_reason from the fixed picklist; a period with 2+ losses sharing a reason renders the pattern as one concern ("both losses cited price"). No `deal_lost` events in period → this source contributes nothing (never backfill from prose).
...
**§7 Appendices** — pipeline by stage, hiring slate, QB financial detail. **Pipeline-by-stage is real data as of SPEC PIPE1:** build it from open deal threads (`deal_state.list_open_deals`) + `pipeline_math` — one row per stage — labeled via `deal_state.STAGE_DISPLAY` (Lead / Qualified / Proposal sent / Negotiating; wire ids never render, UXC1) — with count and summed stated value, plus the period's moves from `deal_stage_changed` / `deal_won` / `deal_lost` events. Untracked deal threads render as one honest count line ("N deals tracked without stages yet"), never invented stages. **Zero deals → DROP the appendix section entirely** (no hardcoded counts, no empty table — G7). Values are user-stated only; a stage with no valued deals shows count-only.

### Phase 4 — Render

Compose the .docx via `shared/scripts/brief_writer.py` board-pack template.

**Deck companion (SPEC OUT6).** When the deck renders — `deck_companion=on`, an explicit deck ask in the trigger ("build board deck", "put together the board deck"), or a yes on `ask` — render it via `shared/scripts/deck_writer.py` `make_deck(...)`, the pptx chokepoint. Pass the SAME assembled `sections` + `exec_header` + `asks` the .docx render used (one assembly, two renderers — the deck can never disagree with its pack), plus `workspace_root` + `org_id` so every color/font resolves through `brand.get_brand()` (org override honored; unconfigured workspace = the premium default theme). Slide structure follows `shared/DECK_GRAMMAR.md` — title-slide verdict (no agenda slide), KPI tile row + compact table (when a §2 `charts` spec renders on this machine, its PNG replaces the compact table beside the tiles — SPEC OUT3; fallback is the table, automatic), one slide each for wins/concerns (5-row cap) and decisions, the three-ask cap, ≤6 bullets per appendix slide with continuation-slide overflow — the caps are pinned in `deck_writer.GRAMMAR`, never improvised. Output path: `_hq/board-packs/BoardPack_[YYYY-MM-DD].pptx` (same stem as the .docx).

Failure posture: on `DeckDependencyError` (python-pptx unavailable and uninstallable) say so in ONE line, deliver the .docx, and STOP — never hand-build a deck around the chokepoint (see GATE1). On `DeckGrammarError` / `DeckLeakError`, fix the sections payload and re-call `make_deck`; maximum 2 retries, then surface the failure plainly and deliver the .docx alone.

Visual pass for the deck (same posture as the .docx pass above): `visual_gate.render_preview(<pptx path>)` takes the pptx→PDF rung (PowerPoint COM → soffice); LOOK at the returned slide images against the 7-item checklist, fix + re-render AT MOST ONCE, then log `visual_gate.log_visual_gate(...)` either way. `None` = no pptx renderer on this machine — log `rendered: false` with a `skipped_reason` and proceed (warn-only forever).
...
Append `board_pack_assembled` event with section counts — and `pptx_path` when the deck companion rendered (the Writer Contract's `pptx_path?` field).
