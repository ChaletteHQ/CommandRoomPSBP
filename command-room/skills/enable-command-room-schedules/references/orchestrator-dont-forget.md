...
- `resolved` (v2.14.1 — DROPPED the `[reason]` bracket per Bo's Apr 30 testing: clicking "Resolved" surprised him with a "why is it resolved" textarea. Per CONTRACT.md Rule 6: action labels must be clear about what they do; surprise inputs violate that. v2.14.1 makes `resolved` a plain state change everywhere — same behavior as Commitments YOU OWE `resolved`. Suppresses the alert for 14 days. Display label: `Resolved`. NO textarea, NO input affordance.
  ...
  - Same display label as Commitments YOU OWE `Resolved`. Same behavior, same mental model: "this isn't open anymore." Now also same mechanic.
