...
- If `today - last_event_date > 30 days` → propose the dormant transition **through the Living Brain rail (LB2 §3a — the migrated writer)**: `brain_proposals.propose(workspace_root, kind="dormancy", tier="confirm", fingerprint="dont_forget:<thread_id>", detector="dont-forget", evidence="<N> days quiet", render_line="this project is going quiet — move to Dormant?", ttl_days=30, thread_id=<thread_id>, action_tuples=[{"action": "confirm proposal"}, {"action": "dismiss proposal"}, {"action": "snooze proposal 7d"}], extra={"title": <project display name>})`. Do NOT append a bare `dont_forget_dormant_proposal` event any more — `propose()` owns dedup (an open row or a pre-migration legacy row with the same target suppresses re-proposing; a dismissal carries the shared 60d cooldown, superseding the old 14d prose cooldown) and the row surfaces in this chat with real verbs. Still surface it in this chat's REVIEW section as before — the rail changes where the row LIVES, not where it shows.

  **STAFFCUT §3.7 (M ruling 2026-08-02) — dormancy rows are ON-DEMAND and no longer appear on the staff meeting.** Whether a quiet relationship has actually gone dormant is a judgment the CEO makes when he asks, not weekly homework on the surface that is now the only door for everything else. The demotion is enforced in the projector, keyed on the KIND, so it covers both this migrated rail and the pre-migration fossil rows: `brain_proposals.load_open_proposals(ws, "on-demand")` is the read that returns them, and every other named surface skips them. Nothing is tombstoned, nothing expires early, and the `active` / `archive` / `snooze 14d` handlers below are unchanged. **THIS CHAT IS WHERE THEY SHOW.** When reading the open dormancy rows for the REVIEW section, call `load_open_proposals(ws, "on-demand")` — a plain `load_open_proposals(ws, "pulse")` now returns none of them.
