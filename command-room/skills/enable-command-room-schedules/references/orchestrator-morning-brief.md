...
- `dark_surface_lines` → digest tail, verbatim, directly beside `watchdog_line` (TASKALARM1 — see Phase 4's map item 4b). Each line already names the task and the fix; never re-narrate, never add a cause.
- `schedule_refresh_announce_lines` → digest tail, verbatim, directly beside `dark_surface_lines` (BRIDGESIL1 — see Phase 4's map item 4c). One line per silently-applied core cadence change since the last brief — "announce, don't ask" (Ruling §0.3); never a confirm ask, never re-narrated.
