...
- `N push to [date]` (displays **Later…**) → parse via `commitment_state.parse_later_when`, then ONE call to `commitment_state.apply_later(workspace_root, <data.id verbatim>, when_iso=<the resolved ISO date>, actor_id=<user person_id>, source_skill="commitments", surface="my-plate")` — never a hand-appended event (APPLYAUDIT1 part 3: a hand-append returns no status, so the Apply receipt booked every push as an error). Every row on THIS chat is the owner's own, so the writer routes to the `commitment_updated` due-date shift (`data.new_due`) and returns `{"status": "deferred"}`; update any staged draft to mention the new date.
