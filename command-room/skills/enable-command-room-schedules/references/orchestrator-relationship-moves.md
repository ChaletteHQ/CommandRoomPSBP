...
print(json.dumps(check_lateness('<workspace_root>', 'relationship-moves', fired_via='<scheduled|manual>', env_date='<session date>')))
"
```
...
Append the fire receipt via the canonical helper (`shared/scripts/receipts.py`, v4.5.2 R1 — **NEVER hand-roll the receipt JSON**): `from receipts import log_receipt; log_receipt(WORKSPACE_ROOT, "relationship-moves", fired_via=<the Phase 2.9 receipt_fired_via: manual|scheduled|catchup>, surfaced=candidate_count, extra_data={"rerun_of": <lateness["rerun_of"], or omit on any other tier>})` — `receipt_fired_via` is what Phase 2.9's helper returned, never guessed. **The `rerun_of` key is SPEC RERUNFAN1:** copy `lateness["rerun_of"]` verbatim on a `rerun` tier and OMIT the key entirely on every other tier, because a re-run receipt written without it re-arms the two-hour skip and the next press is refused. Then STOP — the widget + Links section is the whole turn.
