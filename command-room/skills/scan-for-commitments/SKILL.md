...
- Resolve the mail-thread-fetch tool via `tool_discovery.discover_mail_thread_fetch_tool(tools, declared=connector_config.declared_backend("email"))` — the seam resolves the declared backend first and refuses to substitute another product's tool. Call its `result.tool_id`'s thread/conversation fetch method. Do NOT hardcode a per-installation MCP UUID, and do NOT call the seam bare: on a workspace with two mail connectors the undeclared path reads whichever platform is listed first, and the wrong one returns nothing at all.
...
1. **Discovery gate:** resolve the mail search tool via `tool_discovery.discover_mail_search_tool(tools, declared=connector_config.declared_backend("email"))` (native parity across every provider in the capability manifest — Gmail, Superhuman, Outlook — including UUID-namespaced servers, which it identifies by fingerprint). The seam resolves the declared backend first and refuses to substitute another product's tool. Absent → the Sent pass doesn't exist for this scan (skip-not-fail, zero mentions). Present-but-failing mid-scan → one honest line, the rest of the scan proceeds.
...
- Mail thread fetch — resolved via `discover_mail_thread_fetch_tool(tools, declared=connector_config.declared_backend("email"))`. Native Gmail (`get_thread`) or Outlook (`get_conversation`).
- Mail search (the Sent pass, v4.6.2) — resolved via `discover_mail_search_tool(tools, declared=connector_config.declared_backend("email"))`; the **in-sent-since-floor** intent compiled per provider by `connector_adapters/mail.py` (Gmail operators, Outlook folder query, Superhuman structured filter — never named here). Absent = the Sent pass doesn't exist; present-but-failing = one honest line, scan continues (skip-not-fail).
