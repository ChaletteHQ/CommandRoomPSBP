...
- `end-of-day` (SPEC EOD1 2026-08-16 — the evening bookend's row-list widget, emitted by the 5 PM chat (taskId `past-meetings` until EOD2 renames it) and by the manual `end of day` family. Rows come from the fire's own pack: the **slipped** section then the **needs-your-call** section, numbered continuously in that order. **Fire marker:** the `pack_run` receipt for `past-meetings` carrying `data.surface: "end-of-day"`; its `data.confirm_ids` is `[{n, id, block}]` — the numbered map in render order — and `data.day_intent_proposal` is the tomorrow block's pre-confirm draft.
  ...
  **The tomorrow tap is its own route.** `confirm` / `edit [change]` on the tomorrow block resolves through `end_of_day.resolve_intent_confirm(WORKSPACE)` and writes through `day_intent.write_day_intent(WORKSPACE, <the proposal's item texts>, origin="wrap", for_date=<the proposal's for_date>, source_ref="session:<receipt id>", source_skill="past-meetings")`. `origin="wrap"` because the CEO tapped: it is a stated intent now. The pre-confirm draft is `origin="proposed"` and is NEVER written silently — a guess must not masquerade as the CEO's own word (SPEC BK1). On `edit [change]`, write the CEO's sentence; do not merge it with the draft. If `resolve_intent_confirm` returns `ok: False`, say its `refusal` and write nothing — a re-derived proposal is a NEW guess wearing the CEO's confirmation.
