...
#     gmail_draft_id is the LEGACY param name — pass the DECLARED BACKEND's
#     native draft id (the same value build_email_drafted_provenance dual-wrote
#     onto the event; EW2+T F-12 — a Superhuman draft id goes here too).
snapshot_draft("<abs workspace root>", skill="email-writer", domain="<email domain>",
               ...
               draft_event_seq=<the email_drafted seq>, gmail_draft_id="<backend-native draft id or None>")
...
1. **The `hold` verb** on a row's dropdown (label: *"Hold (14 days)"* — the F-59 rule: the label states its own duration). Dispatch as above.
...
(Pre-v2.14.4 the set was `send` / `edit then send` / `to drafts` / `edit then draft` / `skip`. v2.14.4 consolidated `to drafts` + `edit then draft` → `draft` — the single verb always opens an edit-then-save flow before persisting to the declared backend's Drafts. The renderer rejects the legacy 5-verb shape.)
...
| `<n> edit then send` | Edit then send | Multi-field edit (To/Cc/Subject/Body) → send (same dispatch + writes) |
| `<n> draft` | Draft | Multi-field edit → save to the declared backend's Drafts via the seam-resolved draft tool (§0.5). Appends `email_drafted` (F-12). v2.14.4+ consolidated `to drafts` + `edit then draft` into this single verb. |
| `<n> skip` | Skip | Discard the draft. No connector write; a `chat_dismissal` records the review. |
