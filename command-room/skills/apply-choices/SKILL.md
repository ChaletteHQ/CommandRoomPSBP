...
- `meeting-notes` (on-demand processing, Step 9 OPEN ITEMS section. **ATTRIB1-B door 1 (2026-09-04):** a tuple whose `n` is `<commitment id>:<person id>` with `action: "confirm"` is a WHO-IS-YOU pick from the meeting card's questions widget → `attribution_doors.apply_counterparty_pick(workspace_root, "confirm_counterparty:<commitment id>:<person id>", picked_by=<user person_id>, source_skill="apply-choices")` — ONE call, never a hand-built event: it confirms the counterparty through `reassign_commitment`, clears the flag through `clear_review_flags(confirmed_by="user_pick")`, and appends one extraction hint. Honour `not_pending` (already answered elsewhere — say so, write nothing), `not_an_option` (the person is not one the question offered — route through `reassign to [name]` instead), `no_question` and `malformed` as honest no-ops. Ack: "Got it — <name> it is.")
- `show-my-list` (v2.14.19+ — review surface for `commitment_to_discuss` items; the capture verb `add to my list` is RETIRED (MLK1, 2026-07-21), so this is now a DRAIN-ONLY fossil view: new renders offer `resolved` / `drop` per row and nothing feeds the backlog. **Handlers (Stage B 2026-07, supersedes the v3.11.4 direct-write shape):** `resolved` → close through **`commitment_state.close_commitment(workspace_root, <original commitment_to_discuss event's seq>, resolved_by=<user person_id>, evidence="resolved from my-list review", source_skill="apply-choices", user_confirmed=True, source_ref="session:<the dispatch's session/receipt id>")`** (PROV1 — a chat verb has no message id, so its pointer is the session/receipt the click arrived on; omit it only when there genuinely is none, and since SPEC PROVMINT1 the writer mints `session:apply-choices:<now>` marked `surface_minted` instead of stamping the close as pointing at nothing. That floor points at the act, not at an artifact, and it is not a licence to drop a receipt you HAVE — the coverage split reports minted refs apart from artifact-backed ones) — the normalizer maps the bare seq to the canonical id and the write is the canonical `commitment_resolved` shape (NOT `thread_resolved` with `data.target_id`, the pre-v3.11.4 shape — see references/HISTORY.md). `drop` (MLK1 — the second drain verb) → same `close_commitment` call with `resolution="dropped"`, evidence "dropped from retired-list review". `skip` → write `chat_dismissal` event with `data.target_id` set to the original event's seq (unchanged from pre-v3.11.4 — chat_dismissal is its own canonical pattern; no NEW render offers it here post-MLK1, but persisted old list widgets still dispatch it). For backwards-compat, show-my-list's render filter accepts BOTH the new `commitment_resolved.data.commitment_id` AND legacy `thread_resolved.data.target_id` as valid closers, so in-flight items from pre-v3.11.4 still close correctly. **Mute-ledger rows (v4.6.0 S4 — the `show muted` / `show snoozed` widget, same `src`):** `unmute` → `mute_ledger.clear_dismissal(workspace_root, <row's dismissal seq verbatim>, cleared_by=<user person_id>, source_skill="show-my-list")` — additive `chat_dismissal_cleared`; honor `already_inactive` as a NO-OP with an honest ack ("that one had already expired"); the ack for a real clear says when the item will re-surface ("Unmuted — it'll be back on its next chat."). `skip` on a ledger row → standard `chat_dismissal` (24h) targeting the LEDGER ROW rendering, never the underlying mute.)
- `commitment-triage` (Phase 2 Stage D, S4 — the `triage my commitments` surface + the Friday opt-in scheduled chat. **Dispatch (all through `commitment_state` — NO in-place mutation ever, F4). PLATE1 (2026-09-03): the widget IS the plate; every row's `n` is the commitment's `data.id` off the persisted page, so every close states its door — pass `resolved_by_match="id"` (CLOSEID2).** `resolved` → `close_commitment(workspace_root, <data.id verbatim>, resolved_by=<user person_id>, evidence="triaged: done", source_skill="commitment-triage", user_confirmed=True, resolved_by_match="id")`; on a CONFIRM-block row (`block: "confirm"`) branch on the row's own `pending` flag: a row with **`pending: true`** (an extractor's guess — `data.pending_review`) runs **`needs_review_queue.done_items(workspace_root, [<data.id verbatim>], resolved_by=<user person_id>, source_skill="apply-choices", attested_ids=[<the same id>])`** — the DONE1 twin: it confirms AND closes in one tap (P3/G3), never a bare close of a guess; `source_skill` MUST be `"apply-choices"` (the writer refuses `commitment-triage` — the value is the closure's evidence sentence, and only the dispatcher's own name is accepted there). Every OTHER CONFIRM row — no owner on record, or a system-written question, `pending: false` — is a real item and closes through the ordinary `close_commitment(..., resolution="done", user_confirmed=True, resolved_by_match="id")` above (`done_items` would return it as `n_not_pending` and close nothing — REVIEW_PLATE1_N1 F-1); `drop` → `close_commitment(..., resolution="dropped", evidence="triaged: dropped", resolved_by_match="id")`; `not mine` → **`commitment_state.disown_commitment(workspace_root, <data.id verbatim>, disowned_by=<user person_id>, source_skill="commitment-triage")`** — the owner is CLEARED and a `whose_is_this` question is written with the user's provenance; the item stays OPEN, moves to PARKED on the plate, and the review-expiry drain exempts it (a decline is never a closure — P3). an `already_disowned` or `not_open` result is an honest no-op: say so, write nothing. If the user NAMED the real owner (input or chat phrase), route via `reassign to [name]` below instead of disowning; **the block one-taps (PLATE1 night 2, D4)** — `nudge` on a CHASE-block row (`block: "chase"`) and `follow-up call` on a SCHEDULE-block row (`block: "schedule"`) route to their EXISTING handlers below (the email-writer chase-draft chain and the calendar-invite request draft respectively), unchanged: same wire id, same compose-on-click draft posture, nothing sends and nothing books itself. They are the only two verbs on this surface beyond the four, they appear on no other block, and a row rendered without verbs (the board, the day-close, the wrap, a would-hold read) carries neither. No substrate event is written by either — the draft's own append is the email-writer chain's; `push to [date]` (displays **Later…**, t3 FB-3) → **FIRST the date-less refusal (CUT-C item 5, ATTENDED_TEST_v5.28.0 B2.5):** if `commitment_state.later_needs_input(<input>)` is True — the tuple arrived with no date at all, which only a page NOT relayed byte-exact from the transport can produce (the widget's F-17 hold keeps Apply disabled until the field is filled) — do NOT resolve anything and do NOT call `apply_later`: say `commitment_state.later_missing_ack(<the row's display number>)["ack"]` verbatim as the row's ack (it names the missing input and how to give it — *"Later… on item 7 needs a date — pick a date or a number of days on that row and Apply again. Nothing moved."*), pass that dict as `handler_result` (its `status: missing_when` receipts an error outcome, so the row stays on the page and is re-offered), and NEVER ask a free question in its place ("when do you want it back?" is a new question class; M 2026-09-03: none). Otherwise resolve the date, then ONE writer call. Parse the when via `commitment_state.parse_later_when(<input>, <now_iso>, workspace_path=workspace_root)` FIRST (bare number = days from TODAY, where today is the calendar day `<now_iso>` falls on in the WORKSPACE timezone — pass the clock stamp you hold, UTC or offset-carrying, and ALWAYS pass `workspace_path`: the parser localizes through tz.py, so a push at 18:00 PT counts from that evening's own day, never from the next UTC day (REVIEW_CUTC F-4 — without the workspace a `…T01:00:00Z` now made "7" land on Monday, Sep 14 with the weekday spelled correctly); ISO date passes through; `None` → fall back to the standard natural-language date parse), then **`commitment_state.apply_later(workspace_root, <data.id verbatim>, when_iso=<the resolved ISO date>, actor_id=<user person_id>, source_skill="commitment-triage", surface=<the firing surface>)`** — the writer routes by ownership through `later_route` and performs the landing itself: an item the user OWNS gets its due date moved (`commitment_updated` carrying `data.new_due`, `{"status": "deferred"}`); an item owed TO them (or unowned) gets a dated mute instead (`chat_dismissal` carrying `data.target_id` + `data.snooze_until`, `{"status": "snoozed"}`) — the commitment stays OPEN and simply stops rendering until then, because a view-management click must never rewrite a date the counterparty owns. A closed item returns `{"status": "not_open"}` and writes nothing. **NEVER hand-append either event (APPLYAUDIT1 part 3).** The prose used to describe the two shapes and leave the append to the executing model: a hand-append returns no writer result, so `handler_result` had no `status` and every `push to [date]` in the defect register was receipted an error. `handler_result` is `apply_later`'s return dict — cache its `prior_due` in the fire-marker beside the id (POLICY1-B (b): `null` means the row had no date, and that IS the value to restore). Ack says which leg happened in plain English and RELAYS the writer's `moved_phrase` for the day — *"moved to {moved_phrase}"* vs *"hidden until {moved_phrase} — still theirs to deliver"* — never a weekday you compose yourself (CUT-C item 5: the v5.28.0 ack said "Friday, Sep 13" for a Sunday; `moved_phrase` is `due_reanchor.render_moved_phrase` in the workspace timezone, e.g. "Sunday, Sep 13"); `make task` → `promote_task_to_commitment(workspace_root, <id>, new_kind="task", source_skill="commitment-triage", reason="user triaged to task")`; `promote` → `promote_task_to_commitment(..., new_kind="promise", reason="counterparty appeared — user promoted")`; `never track this` → append the item's suppression pattern (title-shape, one line) to `_hq/config/commitment-rules.md` via atomic write + `close_commitment(..., resolution="dropped", evidence="never-track rule added")`; `skip` → `chat_dismissal` (24h, unchanged). **Lifecycle verbs (v4.6.0 S4):** `fix wording [text]` → `commitment_state.edit_commitment_wording(workspace_root, <data.id verbatim>, new_summary=<input> (or new_title when the item renders title-only), edited_by=<user person_id>, source_skill="commitment-triage")` — the projection re-renders the new text; history keeps the original; ack shows the corrected line; `reassign to [name]` → resolve the typed name to a person via the standard entity-resolve path (aliases first; ambiguous → disambiguation widget, never guess), then `commitment_state.reassign_commitment(workspace_root, <id>, new_owner_id=<resolved person_id>, new_owner_name=<display name>, reassigned_by=<user person_id>, reason="user reassigned in triage", source_skill="commitment-triage", confirmed=True)` — confirmed=True because the user explicitly named the person; the item leaves their you-owe and lands on the new owner (ack: *"Routed to Quinn — it's on their side of the ledger now."*); `split into [items]` → parse the input into 2+ child titles (split on newlines / semicolons / " / "), then `commitment_state.split_commitment(workspace_root, <id>, [{"title": ...}, ...], split_by=<user person_id>, source_skill="commitment-triage", user_confirmed=True)` — N new items each carrying the original's provenance, the original closes with a "split into …" note (ack names the N new items); a parent with open sub-items REFUSES to split (ValueError — surface its message verbatim: close/drop the sub-items first, or `add subitems` to decompose further). **Sub-items (SUB1):** `add subitems [items]` → parse the input with the SAME newline/semicolon/" / " rule (≥1 title is valid here — one named step is a legitimate decomposition, unlike split's ≥2), then `commitment_state.add_subitems(workspace_root, <parent data.id verbatim>, [{"title": ...}, ...], added_by=<user person_id>, source_skill="commitment-triage", user_confirmed=True)` — the parent STAYS OPEN as the commitment of record; N child commitments land under it (widget re-renders them nested with the parent's progress chip). Chat phrases: "break #N into: A / B / C", "add sub-items to #N: …", "steps for #N: …". Loud writer errors surface verbatim: >12 open children (a 13-step item is a project), or a parent that is itself a sub-item (one level deep, no grandchildren). Child rows dispatch like any commitment row on their OWN `data.id` (Done/Later…/Drop/Not mine/Turn into a task/Skip — children are real commitments; `never track this` stays parent-level). **Parent Done with open sub-items (cascade, SUB1 D3):** `resolved`/`drop` on a parent whose row shows open sub-items → `close_commitment` raises `OpenSubitemsError`; NEVER swallow it — ask the one-line confirm ("this also closes its N open sub-items — go ahead?"), and only on the user's yes re-dispatch with `close_subitems=True, user_confirmed=True`. The cascade closes children first, parent last, and returns `closed_subitems` (the child ids). When the LAST open child closes normally, the parent row shows "all sub-items done — close it?" — that is a PROPOSE, never an auto-close. **Undo (S4):** cache the batch's closed/reclassified ids — INCLUDING every id in a cascade's `closed_subitems` return (SUB1: an undone parent-cascade must reopen the whole family; reopen is per-item, each child has its own tombstone) — AND the seqs of every `chat_dismissal` the batch wrote in the fire-marker; a follow-up `undo` in the same chat reopens each closed item via `commitment_state.reopen_commitment(workspace_root, <id>, reopened_by=<user person_id>, reason="triage undo", source_skill="commitment-triage")`, puts each deferred item's date back EXACTLY via `commitment_state.restore_due(workspace_root, <id>, prior_due=<the cached prior_due — pass `None` verbatim when it was none>, actor_id=<user person_id>, reason="triage undo", source_skill="commitment-triage")` (POLICY1-B (b): the sanctioned clear — a row that had no date goes back to no date; never say a date cannot be cleared), hands each disowned item back via `commitment_state.confirm_commitment_owner(workspace_root, <id>, owner_id=<the disown result's previous_owner_id>, confirmed_by=<user person_id>, source_skill="commitment-triage", reason="triage undo")` (cache `previous_owner_id` from each `not mine` result; an empty one means the item had no owner before — leave it), reverses each reclassification with an opposite `promote_task_to_commitment` call, AND clears the batch's mutes via `mute_ledger.clear_dismissals(workspace_root, <cached dismissal seqs>, cleared_by=<user person_id>, source_skill="commitment-triage")` (the F-20 P3a asymmetry, fixed — undo used to reopen items while leaving their mutes in force). Additive only — tombstones, markers, and clears stay in history. Ack: *"Reversed — N items back open, M mutes lifted."*)
...
- `command-room-onboarding` (v3.4.1+ — first-install Step 1 setup widget. Fire-marker event type: `onboarding_setup_widget_emitted` with `source_skill: command-room-onboarding` and `data.widget_kind` of `step_1_setup` (pre-v2, 4 tuples), `step_1_setup_v2` (3 tuples: role / email exclusions / timezone), `step_1_setup_v3` (4 tuples, M1: + AI name), `step_1_setup_v4` (5 tuples, SPEC FRP1: role / email exclusions / timezone / AI name / email draft posture), `step_1_setup_v5` (4 tuples — role / timezone / AI name / email draft posture; the email-exclusion question was dropped 2026-06-30), or `step_1_setup_v6` (current: 5 tuples — + check-in cadence, SPEC QUIET1 D1; `daily` / `few_times_a_week` / `rarely` ALL stamp the commitment-policy preset `light` through `quiet.stamp_onboarding_preset(...)` (origin `m1_batch`, receipted and undoable — NOT entities.json; CUT-D, M ruling 2026-09-06: every seat starts light, the only way up is `ask me more`). Source identification keys on `source_skill`, NOT `widget_kind` or tuple count, so all payload shapes dispatch. Item 1 may carry an optional `sub` field from the role question's sub-chip drill-down, and any item may carry an optional `input` from a refinement textbox. Dispatch routes to `skills/command-room-onboarding/SKILL.md` Section "Reply handling — Phase 0 setup" — that section maps each `(action, sub?)` pair to the appropriate write by topic: role / timezone / AI name to entities.json `workspace.*`, and email draft posture (`show_first` / `auto_queue`) to email-writer's `skill_config` via `save_skill_config(..., origin="m1_batch")` — NOT entities.json. A legacy v4 payload's email-exclusion tuple is ignored. Then resumes onboarding flow control at Phase 1. The actions are selection labels, not action verbs from `CANONICAL_ACTIONS` — apply-choices Step 3 parses this payload but does NOT validate the actions against `CANONICAL_ACTIONS` for this source.)
- `morning-brief` (P0.7 2026-07 — the Morning Brief chat's `mark done [n]` affordance, morning-briefing Step 3b / Bug #85. Morning Brief is a markdown digest, not a widget — the action normally arrives as a typed `mark done [n]` reply in that chat; treat it exactly like a widget tuple with `src: "morning-brief"`. Fire-marker event type: `pack_run` with `source_skill: "morning-brief"`; its `data.needs_attention_ids` records the commitment id (`data.id` verbatim) for each numbered Needs Attention item in render order. **Handlers:** `mark done [n]` → resolve `[n]` through `brief_receipt.resolve_mark_done(workspace_root, n)` — the ONE resolver, never a hand-read of the receipt — then close the returned id through `commitment_state.close_commitment(workspace_root, <that id verbatim>, resolved_by=<user person_id>, evidence="marked done from morning brief", source_skill="apply-choices", user_confirmed=True)` — the canonical closure path (Stage B); never a hand-built `commitment_resolved` append. **No `source_ref` here, and that is checked, not forgotten (PROVMINT1):** `brief_receipt.resolve_mark_done` returns `{ok, id, refusal, n}` and hands back no receipt id, so nothing better than the surface itself is in scope on this path. The writer mints `session:apply-choices:<now>` and marks it `surface_minted`, which is the true grain of a typed `mark done [n]`. Do not invent a brief id to put here.

  ```python
  from brief_receipt import resolve_mark_done
  target = resolve_mark_done(WORKSPACE, n)
  if not target["ok"]:
      # say target["refusal"] verbatim and write NOTHING
      ...
  ```

  **The resolver REFUSES rather than guessing, and its refusal is the answer (SPEC BRIEFFIX1 Item C).** Three cases, all handled inside it: no recorded numbering at all, `[n]` outside the recorded list, and — the one that matters — a recorded numbering OLDER than the newest brief. That last case is the 2026-08-09 shape: a brief posted without recording its receipt, so the newest numbering on file belongs to a DIFFERENT brief and position 3 is a different item than the one on screen. Closing anyway would close real work silently, which is worse than refusing. When `ok` is False, say `refusal` verbatim, write nothing, and do not offer to "try the closest match". No other actions dispatch from this source.)
- `end-of-day` (SPEC EOD1 2026-08-16 — the evening bookend's row-list widget, emitted by the 5 PM chat (taskId `past-meetings` until EOD2 renames it) and by the manual `end of day` family. Rows come from the fire's own pack: the **slipped** section then the **needs-your-call** section, numbered continuously in that order. **Fire marker:** the `pack_run` receipt for `past-meetings` carrying `data.surface: "end-of-day"`; its `data.confirm_ids` is `[{n, id, block}]` — the numbered map in render order — and `data.day_intent_proposal` is the tomorrow block's pre-confirm draft.

  **Every action on this source resolves through ONE resolver — `end_of_day.resolve_choice` — never a hand-read of the receipt:**

  ```python
  from end_of_day import resolve_choice
  target = resolve_choice(WORKSPACE, n, action=<the verb verbatim>)
  if not target["ok"]:
      # say target["refusal"] verbatim and write NOTHING
      ...
  ```

  It refuses in four cases and each refusal IS the answer: no End of Day has ever recorded a numbering; the newest fire recorded none while an older one did (the stale-map case — position 3 on screen is a different item than position 3 on file); `[n]` outside the recorded list; and a verb offered on a block that never carried it. Never clamp, never "try the closest match", never re-derive the list.

  **Handlers, once `target["ok"]`:** `mark done` / `resolved` → `commitment_state.close_commitment(WORKSPACE, target["id"], resolved_by=<user person_id>, evidence="marked done from the End of Day", source_skill="apply-choices", user_confirmed=True, source_ref="session:<the dispatch's session/receipt id>")` — the canonical closure path WITH its PROV1 pointer, never a hand-built append (SPEC EOD1: every close this fire writes carries a `source_ref`); `push to [date]` → parse the when with `commitment_state.parse_later_when(<input>, <now_iso>, workspace_path=WORKSPACE)` (today = the workspace-local day of `<now_iso>`), then ONE call to `commitment_state.apply_later(WORKSPACE, target["id"], when_iso=<the resolved ISO date>, actor_id=<user person_id>, source_skill="apply-choices", surface="end-of-day")` exactly as `commitment-triage` does — the writer routes by ownership (own item → the due-date move, `deferred`; owed-to-you → a dated mute, `snoozed`) and its return dict IS the `handler_result`; never hand-append either event; `draft` → hand off to `email-writer` honoring the EW1 delegation rules (lazy draft, nothing sends); `drop` → the same `close_commitment` call with `resolution="dropped"`, evidence "dropped from the End of Day"; `confirm` → the existing confirm path for that row's class, unchanged (CAPTUREFLOW's unified confirm — this surface relocated the selection, it did not respec the adjudication).

  **The tomorrow tap is its own route.** SPEC TOMPICK1: up to three ranked candidates render, and the CEO picks ONE, positionally — a bare `confirm` takes rank 1 (the default door, unchanged), a bare digit (`1`/`2`/`3`) takes that rank instead. `confirm` / `1`|`2`|`3` / `edit [change]` on the tomorrow block resolves through `end_of_day.resolve_intent_confirm(WORKSPACE, pick=<the digit typed, or None for a bare "confirm">)`, which already narrows the proposal to the ONE chosen candidate before returning it. Write it through `day_intent.write_from_proposal(WORKSPACE, res["proposal"], origin="wrap", source_ref=res["source_ref"], source_skill="past-meetings")` — never a hand-built list of item texts, and never `write_day_intent` directly: `write_from_proposal` is what keeps the `commitment_id` the candidate was drafted from, and `for_date` comes off the proposal, not a fresh "tomorrow" resolve. `origin="wrap"` because the CEO tapped: it is a stated intent now. The pre-confirm draft is `origin="proposed"` and is NEVER written silently — a guess must not masquerade as the CEO's own word (SPEC BK1). On `edit [change]`, write the CEO's sentence; do not merge it with the draft. If `resolve_intent_confirm` returns `ok: False` — a stale/absent map, or a pick outside the offered range — say its `refusal` and write nothing — a re-derived proposal is a NEW guess wearing the CEO's confirmation.

  **A HELD capture never appears here.** The fire's confirm block excludes them by construction (`end_of_day.compute_confirm(..., held_ids=...)`), so there is no held row to act on; if the user asks about one, `held_tier.load_held` is the retrieval path and it is read-only.

  **Person-candidate rows (SPEC PERSONLOOP1) ride the confirm block and resolve the same way.** `resolve_choice` returns `block: "person_candidate"` and, for those rows only, a `data` payload (`name_key`, `candidate_name`, `observed_spellings`, `org_id`) — a candidate is DERIVED live from the pending queue and has no substrate id, so the row carries its own payload and dispatch stays stateless. Their three verbs are legal on that block and nowhere else, and they are handled exactly as the PERSONLOOP1 entry below specifies. A `drop` or `mark done` on a candidate row is REFUSED by `ROUTES`, which is correct: there is nothing to close.)
- `decision-revisit` (P1.1 2026-07 — the revisit widget's per-decision deliberation set. SKILLMERGE1 2026-09-03: the surface is now decision-log's revisit mode; the widget `src` stays `decision-revisit` because persisted widgets carry it. Each item embeds the original decision event's id/seq verbatim. **Handlers:** `revisit` → invoke `decision-memo-composer` pre-filled with the original decision's framing + the contradictory-signal pass (no event write here — the composer's flow owns downstream writes); `still valid` → append `decision_reaffirmed` referencing the original decision id (locked writer; omit seq — auto-stamped); `replace` → chain to `decision-log` to capture the new decision, then append `decision_superseded` linking the original; `snooze 30d` → append `decision_revisit_scheduled` with `snooze_until_ts = now + 30d`; `skip` → `chat_dismissal` (24h).)
...
- `cr-brain` (SPEC LB1 2026-07 — the Living Brain "Needs your eyes" card (morning brief / coach) and the Staff Meeting full-queue widget. Rows come from `brain_proposals.load_open_proposals` / `select_confirm_card` — ONE queue mixing brain-family (`bp_*`) proposals with adapter-read legacy items; each row embeds its proposal id + target ids VERBATIM and carries `context.kind` naming its family. Dispatch is per-kind — brain rows resolve through `brain_proposals.resolve_proposal`; legacy rows dispatch through their OWN shipped flows (the adapters are permanent fossil readers — LB2 migrated the org/project/dormancy/schedule_add WRITERS onto the bp rail, so NEW rows of those kinds arrive as `bp_` ids and dispatch through the brain-family handlers below; only pre-migration rows still arrive as legacy ids. person and commitment_review remain legacy-written — LB3). **Handlers — brain family (id starts `bp_`):** `confirm proposal` → apply the proposed change through its class's single writer FIRST, then `brain_proposals.resolve_proposal(workspace_root, <bp_ id verbatim>, "applied" (or "edited" when input was typed), resolved_by=<user person_id>, source_skill="apply-choices")`. Per kind: `deal_update` → `deal_state.set_stage` / `update_deal` / `close_deal` per the proposal's `proposal_kind` (won/lost only via `close_deal`; `mark lost` requires the reason enum exactly as on `cr-pipeline`); `deal_creation` → **coverage check FIRST (FS-18b):** run `deal_state.org_deal_coverage(<entities threads>, <org_id verbatim>)` — the SAME predicate the detector consults; if it returns a thread, the org's deal activity is already tracked (an open deal or an active engagement thread), so do NOT create: resolve the proposal **`superseded`** (review F-4 — the user clicked Confirm, they did NOT decline; `superseded` is the truthful label for "the premise no longer holds", and it carries the SAME 60d cooldown as declined so the covered item never re-proposes) with a note naming the covering work (e.g. "already covered by [thread display name]"), and ack honestly (*"That one's already tracked — [name] covers it, so I closed the suggestion."*). This also self-heals any pre-T2.2 zombie proposal still open in a live queue. Uncovered → `deal_state.create_deal(workspace_root, name=<proposed>, org_id=<org_id verbatim>, stage=<proposed stage or "lead">, source_skill="apply-choices")` — value only when the proposal carried a user-visible amount, never estimated; `person_update` (HIST1 Part 2 — an entity-signal row about a person ALREADY on file; the row embeds `data.person_id` verbatim + `data.proposal_kind`): `confirm proposal` with `proposal_kind: role_change` → `people_writer.update_person(workspace_root, <data.person_id verbatim>, role=<data.proposed_role — or the user's typed input when they corrected it>, source_skill="apply-choices")`; with `proposal_kind: org_change` → `update_person(..., primary_org_id=<data.to_org_id verbatim>, ...)` — in BOTH cases the writer's Part 1 hook auto-emits the `person_role_changed` / `person_org_changed` lineage event (the old value is preserved as history; do NOT also record a fact for the same change), then `resolve_proposal(..., "applied")`; `kind: entity_fact` (HIST1 Part 2 — a prose-inferred or identity-adjacent fact observation; the row embeds the target id + `data.fact` + `data.category` + `data.source_ref`): `confirm proposal` → `people_writer.record_person_fact` / `org_writer.record_org_fact` (`workspace_root, <target id verbatim>, <data.fact — or the user's edited wording from input>, <data.source_ref, falling back to "proposal:<bp_ id>">, category=<data.category>, source_skill="apply-choices"`) — an additive event, never a record field — then `resolve_proposal(..., "applied")`; `kind: org_money` (HIST1 Part 2 — an observed account/contract value near a client org; the row embeds `data.org_id` verbatim + `data.proposed_money`): `confirm proposal` → `org_writer.set_org_money(workspace_root, <data.org_id verbatim>, <data.proposed_money — the user's typed amount wins over the proposed one>, source_skill="apply-choices", confirmed=True)` (`money.source` = the proposal's evidence ref — money is always sourced, never estimated; the user's click IS the confirm that makes `confirmed=True` truthful), then `resolve_proposal(..., "applied")` — dismiss/snooze for all three kinds are the standard bp handlers below. An empty input applies the proposal as-is and the ack says so (F-17: a missing optional input NEVER holds the batch); **LB2 migrated-kind confirms (same per-kind writers the legacy dispatch below names — migration changed where rows come from, not what clicking them does):** `org` → `org_writer.create_org` (dedup-first through `find_existing_org`), then `resolve_proposal(..., "applied")`; `project` → workspace-manager's new-project writer, then `resolve_proposal(..., "applied")`; `dormancy` → the dont-forget orchestrator's status handlers (`status_change` to dormant via the canonical writer), then `resolve_proposal(..., "applied")`; `schedule_add` → the row is a POINTER — registration happens ONLY through the change-schedule / registration add path, never from this dispatcher; on the user's add, `resolve_proposal(..., "applied")` closes the row; `config_drift` (LB2 §3b — the row re-offers ONE drifted setting, staff-meeting only) → append ONE `note` event via `event_gate.append_event` with `data: {skill: <row's data.skill verbatim>, knob: <data.knob verbatim>, origin: "drift_reoffer", summary: <the row's render_line>}` — the next coach session consumes it and re-offers THAT KNOB through the tune flow — then `resolve_proposal(..., "applied")`; **apply-choices NEVER writes `skill_config` from this row** (the tune flow is the only config writer; cleanup stays read-only on prefs); `kind: objective_link` (OBJ2 §2 — the staff-meeting "link it?" row from the objective-link detector; the proposal payload embeds `objective_id` + `target_id` + `source_event_seq` verbatim): `confirm proposal` → the CONFIRMING reclassification+feedback pair (OBJ2-R, the mirror of dismiss's — the two writes always land together), THEN the tombstone: (1) read the source event by the payload's `source_event_seq` verbatim and append ONE `reclassification` event via `event_gate.append_event` — `source_skill: "apply-choices"`, `supersedes_seq` = that seq, envelope = the source event's own envelope UNCHANGED (primary and related ids verbatim — a confirm asserts the classification, it never moves it), `classification_confidence: 1.0` (the user's click IS the confidence), `data: {old_primary_thread_id, new_primary_thread_id (equal), old_related_thread_ids, new_related_thread_ids (equal), reason: "user confirmed objective-link proposal <bp_ id> — link to <objective_id>"}`; (2) append the MATCHED `classifier_feedback.jsonl` row (`user_action: "confirmed"`, `new_primary` = old_primary per the schema, `confidence_before` = the source event's `classification_confidence` — `0.0` when the event carries none (a pending_review-only provisional; the schema requires a number), `notes: "objective link confirmed: <objective_id>"`); same half-pair rollback rule as dismiss; (3) `brain_proposals.resolve_proposal(workspace_root, <bp_ id verbatim>, "applied", resolved_by=<user person_id>, source_skill="apply-choices")` — the applied tombstone (the detector's confirmed-fingerprint dedup consumes it, so the pairing never re-lists; still no `objective_state` call — `pending_review` is stamped, never unset, by design, and rebind is the wrong contract). The supersession-aware movement read (`thread_activity.apply_reclassifications`, consumed by `objective_math` + the detector, and since RECL1 by every recency surface via the `honor_reclassifications` seam) is what makes this click COUNT: the confidence-1.0 envelope clears the activity floor, and the objective's own thread joins the movement read. A pre-OBJ2-step-2 row whose payload lacks `source_event_seq` skips the pair-writes and just resolves applied. Ack in plain link language (*"Linked — noted as a move on [objective name]."*); `dismiss proposal` on an objective_link row → the standard reclassification+feedback pair (the insight-generator Pass 8 idiom — the two writes always land together), THEN the decline: (1) read the source event by the payload's `source_event_seq` verbatim (NEVER re-derive it at dispatch time) and append ONE `reclassification` event via `event_gate.append_event` — `source_skill: "apply-choices"`, `supersedes_seq` = that seq, envelope = the source event's own envelope with the row's objective thread removed (the objective was the primary → new primary is `null`; otherwise it leaves `related_thread_ids` and the primary stands), `classification_confidence: 1.0`, `data: {old_primary_thread_id, new_primary_thread_id, old_related_thread_ids, new_related_thread_ids, reason: "user declined objective-link proposal <bp_ id> — unlink from <objective_id>"}` — never edit the original event; (2) append the MATCHED `classifier_feedback.jsonl` row via `atomic_append_jsonl`, schema-valid (`{ts, event_seq: <source_event_seq>, user_action: "changed", old_primary: <the source event's primary>, new_primary: <the new primary from (1)>, signals_used: [], confidence_before: <the source event's classification_confidence — 0.0 when the event carries none (a pending_review-only provisional; the schema requires a number)>, notes: "objective link declined: <objective_id>"}`); if the reclassification append fails, write NO feedback row and surface in `_hq/CONFLICTS.md` (the Pass 8-K rollback rule — never a half pair); (3) `brain_proposals.resolve_proposal(..., "declined", ...)` — the tombstone + the shared 60d cooldown keep the pairing away. A re-dispatched row that is already adjudicated (absent from `load_open_proposals`) skips the pair-writes entirely and goes straight to `resolve_proposal` — its `already_resolved` return is the honest no-op; the pair never duplicates. A pre-OBJ2-step-2 row whose payload lacks `source_event_seq` also skips the pair-writes (nothing to supersede against) and just declines. `handler_result` for EVERY objective_link action is resolve_proposal's return — the pair-writes are never passed as handler_result (FS-18a). Dismiss ack: *"Dropped it — I won't suggest that pairing again."* (the unlink is honored at read time — `objective_math`, the detector, and since RECL1 every recency surface (stalled-projects, Pulse, the master tracker, list-active, the workspace map, DCC, deal rot, org history, `go [name]` ranking) fold reclassifications via `thread_activity.apply_reclassifications`, so a dismissed pairing stops counting everywhere and can never re-propose even after the 60d cooldown lapses; OBJ2-R closed the objective-reader gap, RECL1 the rest); `snooze proposal 7d` → the standard bp snooze below; `kind: thread_split` (SPEC THREADANN1 §0 ruling 4 — the weekly cleanup job's "this thread looks like N things — split it?" row; the payload embeds `parent_thread_id` + `clusters` (the SAME cluster assignment the annotation already carries — never re-derived at dispatch time) verbatim): `confirm proposal` → `thread_subjects.execute_split(workspace_root, <data.parent_thread_id verbatim>, <data.clusters verbatim>, source_skill="apply-choices", user_confirmed=True)` — this is the ONLY caller in the plugin allowed to pass `user_confirmed=True` (the function raises without it; a split is never auto-executed) — THEN `brain_proposals.resolve_proposal(workspace_root, <bp_ id verbatim>, "applied", resolved_by=<user person_id>, source_skill="apply-choices")`. `handler_result` is `execute_split`'s return (`batch_id`/`batch_ref`/`children`/`n_retagged`) — the confirming tap IS what makes the split real, so the ack names what happened, not just that the row closed (*"Split into [N] threads: [labels]. Say 'undo' to put it back."*). **A re-dispatched row that is already adjudicated (absent from `load_open_proposals`) skips `execute_split` entirely and goes straight to `resolve_proposal`** — its `already_resolved` return is the honest no-op; a stale confirm must never split the same thread twice (REVIEW THREADANN1 F2 — same idiom as the objective_link stale-row guard above; `execute_split`'s own stale-seq refusal, which raises on a thread whose clustered events have already moved, is the code-side belt — surface its message, never retry). `dismiss proposal` on a `thread_split` row → the standard bp decline below — no split-side write at all (the clusters stay ONLY as the standing `thread_annotation`; a declined split does not un-annotate the thread, and the standing `thread_subjects._split_proposed_recently` 30-day floor — independent of this row's own 60d decline cooldown — keeps the same thread from re-proposing for 30 days regardless of how this resolves); `dismiss proposal` → `brain_proposals.resolve_proposal(..., "declined", ...)` — the tombstone + the shared 60d cooldown row; deal-kind declines also emit `deal_update_dismissed` (handled inside resolve_proposal — do not append it here); `snooze proposal 7d` → `chat_dismissal` with `data.snooze_until` = now + 7 days (snooze_until is the field the mute ledger honors — a bare TTL number silently falls back to the 24h legacy default) and `data.target_id` = the proposal id verbatim. **DIGEST ROWS (STAFFCUT 2026-08-02) — one row, N members, per-id dispatch. Read this BEFORE the per-kind handlers below.** The staff-meeting driver groups each EVIDENCE CLASS into one row: the audit day's 54 sent-match review rows rested on 16 distinct evidence lines (34 of them on a single line), which is 16 decisions asked 54 times. A digest row is recognisable by its wire id — **`n` starts with `digest:`** — and it carries `data.digest_class` (`commitment_review` / `person_name_only` / `entity_fact`), `data.digest_count`, and **`data.digest_members`: the full per-member dispatch payload, verbatim**. Rules, all of them load-bearing:

- `held-review` (SPEC_HELDREVIEW1 2026-08-23 — the "show me what you'd hide" surface, `src: "held-review"`, emitted by `held_review.render_review_page`. Each row's `n` is the commitment's `data.id` verbatim and carries **exactly one verb, `wrong to hide`**. **Handler:** `held_review.capture_objection(workspace_root, <the id verbatim>)` — ONE `held_review_objection` event, and that is the whole dispatch. The id is all it needs and all the row carries: the reason the capture floor refused it is on the commitment event itself, so re-deriving it here from the rendered `context_tag` would be reading a display string back as data. It writes no closure, no confirm, no review-flag change and no mute; the row stays exactly where it was, in the state it was in. A second objection on the same row returns `already_captured` and writes NOTHING — relay that in one line and never re-issue it. **`confirm`, `already done`, `drop` and `not mine` are NOT on this source and must never be dispatched from it** (SPEC_HELDREVIEW1 DD-4): those rows also live in the needs-your-call queue, which is where they are answered — a surface asking "is this list acceptable?" that could also resolve a row would make reading the list destructive. **Nothing on this source ever turns the held routing on.** The enable is a typed phrase handled by `held-review` Step 3 through `held_review.turn_on_held`; Apply means "carry out my choices about these rows", and a routing change is not a choice about a row. **`skip` and `skip all` are NOT on this source either, and are the ones that actually arrive** — the shared multi-item widget footer renders `Snooze rest (1 day)` on every page including this one, and its handler stages a bare `skip` on every row that carries any action and dispatches immediately, without Apply. Dispatching it here would mute the very rows the user is being asked to judge, on a surface whose shipped promise is that looking changes nothing and that no row is ever muted by it (`skills/held-review/SKILL.md`). Refuse it in one line — *"Looking at this list doesn't change any of it — nothing was snoozed."* — and write nothing. **No other actions dispatch from this source.**)

- **NEVER dispatch a `digest:` id as a proposal id.** There is no proposal behind it. `brain_proposals.resolve_proposal` will refuse it and should. **Expand it first:** every member dict in `data.digest_members` carries its own `id` (a real `cru:` / `person:` / `bp_` id), its own `actions`, and the exact keys its handler reads. Dispatch each member through the handler its OWN kind names below — the grouping changed the presentation, not the adjudication.
- **`show these` on a review digest (BUG-8330 item 9) — the digest's affirmative, and it EXPANDS.** No write, no close, no fence: re-render the row's `data.digest_members` as individual rows (each member dict is already projector-shaped — feed them through `brain_proposals.build_card_view` + the canonical transport as their own page) so each item gets its OWN Close it / Not relevant / Hold. The old digest-level `confirm` closed every member off one click labeled "Close it" singular; new review digests no longer emit it. If a persisted LEGACY widget dispatches `confirm` on a `digest:` id, honor it through `confirm_review_digest` exactly as before — old pages must keep working — but never compose a new digest-level close.
- **A grouped answer is a BULK gesture.** For `digest_class: "commitment_review"` the confirm path is **`proposal_digests.confirm_review_digest(workspace_root, <the batch's rows, digests included>, resolved_by=<owner_id>, individually_named=<only the wire ids the user named on their own>, source_skill="apply-choices")`** — that function expands the digests and hands the flat batch to `watch_gate.confirm_review_rows`, THE same shared fence the ungrouped batch has always called. It is a CALLER of the fence, never a second fence. A weak-evidence member is HELD and PARKED inside it exactly as `confirm all` would be, and **a `digest:` id typed on its own does NOT name its members individually** — the function strips digest ids from `individually_named` for you, and you must never re-add them.
- **`not relevant` / `hold` on a review digest** fan out per member id through the same two handlers named below (one `commitment_review_dismissed` per member for `not relevant`; one `mute_ledger.hold_item` per member id for `hold`). `digest_class: "person_name_only"` carries `proposal not relevant` (a `not_relevant` tombstone per member, and per `cluster_seqs` entry within each member — the PID1 fan-out, unchanged) and `snooze proposal 7d` (one `chat_dismissal` per MEMBER row id, never the digest id — a dismissal keyed on a digest id would quiet a whole class and no reader honors it). It deliberately does NOT carry `add person`: creating N contacts from one click is not a decision anyone made. The row's own copy says how — name one, and its own row dispatches. `digest_class: "entity_fact"` carries the standard bp trio, fanned out per member `bp_` id.
- **One id resolves ONCE.** If the user's selection names both a digest and one of its members, the member is resolved once — `proposal_digests.expand_review_rows` de-duplicates by wire id, keeping the first position and preferring the candidate that carries `evidence`, so a batch carrying both shapes of one row answers the same either way. Never resolve a member twice, and never leave one unresolved because it was "covered by the group."
- **A digest row that arrives WITHOUT its members is REFUSED, never guessed at.** A `digest:` id is not a commitment id. `confirm_review_digest` reports it back on `refused` / `n_refused` and as a `digest_members_missing` entry in `results`; nothing is written for it. Say so in one plain line — *"One grouped row came through without its items, so I left it alone — say `staff meeting` for a fresh page."* — and never fall back to treating the digest id as an item. This is reachable without any defect of ours: a stale frozen page-set or a truncated persisted widget is enough.
- **Undo is unchanged.** A grouped accept closes the same commitments the same way single accepts do, so the batch undo below reverses it identically — cache the closed ids and reopen via `commitment_state.reopen_commitment` exactly as for singles. There is no digest-specific undo.
- **Ack in the returned counts** (`ack` from the fence, or the queue's own kept/dropped/held counts). Never narrate the grouping, never name a `digest:` id, never restate a member list.

**Handlers — legacy families (PRE-migration fossil rows plus the still-legacy-written person / commitment_review families — route to the family's existing dispatch, exactly as its own surface would):** `kind: commitment_review` (LIFECYCLE1 — the Pulse review section is gone; the CLOSURE path below is the one that used to be written down there, and it is unchanged: `resolved` → `commitment_state.close_commitment(workspace_root, <underlying commitment_id verbatim>, resolved_by=<the commitment owner>, evidence=<the review proposal's evidence>, source_skill="apply-choices", user_confirmed=True)` — the explicit click IS user confirmation, so this closes even a `pending_review`-flagged commitment; never hand-build the `commitment_resolved` append. **No `source_ref` here, and that is checked, not forgotten (PROVMINT1):** the proposal row this leg reads carries `evidence` / `match_score` / `evidence_ts` / `promise_ts` and no artifact pointer — `matched_ref` is populated by no writer today (it rides the PARK path as free text), which is why the sibling batch path guards it behind a prefix check rather than trusting it. The writer's minted surface receipt is the honest floor; wire a real ref here the day a writer starts populating one. `not relevant` → `commitment_review_dismissed` with the 60-day cooldown; the underlying commitment STAYS OPEN) → `confirm` NEVER closes row by row from here (WATCHGATE §2.2 — one gesture once turned six of these into "done" in two seconds, every one of them resting on nothing but a title match). Collect EVERY confirmed `commitment_review` row in the batch and dispatch them in ONE call: **`watch_gate.confirm_review_rows(workspace_root, rows, resolved_by=<owner_id>, individually_named=<only the wire ids the user named on their own>, source_skill="apply-choices")`** — THE shared bulk-accept fence, the same one the needs-your-call queue calls. Each row: `{"id": <wire id verbatim>, "commitment_id": <verbatim>, "evidence": <the row's `evidence`>, "match_score": <the row's `match_score`>, "has_completion_signal": <the row's `has_completion_signal`>, "evidence_ts": <the row's `evidence_ts`>, "promise_ts": <the row's `promise_ts`>}` — **every one of those keys is already ON the queue row** that `brain_proposals.load_open_proposals` handed you; copy them across verbatim and derive none of them. A key the row does not carry is passed as `None`, which means "not assessed" and weakens nothing. (The row also carries `strength` and `weak_reason` — that is the badge the user already read, not an input; do not recompute it.) Inside that call a STRONG row closes through `commitment_state.close_commitment(..., user_confirmed=True)` exactly as before, and a WEAK row — no evidence, title-match-only evidence, no completion signal, or an ordering that says the meeting has not happened yet — is HELD and PARKED on watch instead. `individually_named` may carry a wire id ONLY when the user typed that row's own number or name as a standalone answer; "Apply all", a range and a group phrase name nothing individually, so they pass an EMPTY set. **Never re-issue the call with the held ids in `individually_named` on your own** — that override belongs to the user's next sentence. `handler_result` for each row is its own entry in the returned `results` (`status` is `closed`, `watching`, or a refusal). Ack from the returned `ack` line: what was confirmed, what is on watch, and that naming one by its own number keeps it now — never scold, never explain the fence; `not relevant` → `cru_match.build_commitment_review_dismissed_event` + `event_gate.append_event` (60d, the commitment STAYS OPEN); `hold` → `mute_ledger.hold_item(workspace_root, <row id verbatim>, source_skill="apply-choices", surface=<the firing surface>)` (FB-19 — 14d, the commitment STAYS OPEN; see the hold handler below); `kind: person` (T2.2/FS-17 — the enriched identity rows carry `add person` / `proposal not relevant` / `snooze proposal 7d`): `add person` → **`people_writer.auto_add_person(workspace_root, canonical_name=<the proposal's name>, email=<ONLY an address observed in the proposal's own evidence/source_ref text>, email_provenance=<the observed source ref — REQUIRED whenever email is passed; a pattern-guessed or constructed address is NEVER written (F-08 extends to capture)>, role=<inferred_role or None>, source_skill="apply-choices")`** — the same-name dedup gate runs INSIDE the call; a `needs_confirm` return renders the Step 3a disambiguation widget instead of writing, and the ack says so honestly. On `added`, append the `person_added` tombstone via `confirm_flow.build_person_proposal_resolved_event` (the entity write FIRST, then the tombstone — never skipped). **Cluster fan-out (PID1 D3):** a person row may embed `data.cluster_seqs` (plus `data.cluster_fingerprints` for seq-less legacy rows) — the identity-clustered row covering EVERY underlying proposal for that person. `add person` runs **ONE** `auto_add_person` call, then a tombstone **per** entry: one `build_person_proposal_resolved_event(<seq>, resolution="person_added", person_id=<new id>)` per int in `cluster_seqs` and one `build_person_proposal_resolved_event(None, ..., proposal_fingerprint=<fp>)` per entry in `cluster_fingerprints`; `proposal not relevant` → a `not_relevant` tombstone per entry the same way; `same as [existing]` → ONE `add_person_alias` then a `same_as` tombstone per entry. When the row carries NO `cluster_seqs`, fall back to the single embedded seq — back-compat with in-flight pre-PID1 widgets. Free-text `input` on the row folds in as the org/role correction exactly as the W4b `add person` handler specifies. `same as [existing]` stays available (chat / the commitments confirm section) and dispatches per W4b (alias add + `same_as` tombstone); `proposal not relevant` → the `not_relevant` tombstone via the same builder; `snooze proposal 7d` → `chat_dismissal` with `data.snooze_until` = now + 7 days and `data.target_id` = the `person:NNN` row id verbatim (one dismissal per ROW — a snooze quiets the whole cluster row, it adjudicates nothing); **`kind: person_link` (PID1 D4a — a `bp_` row: "Looks like [name] is already on file as [record] — link it?"; the row embeds `data.person_id` = the matched record, `data.alias_name`, `data.cluster_seqs`/`data.cluster_fingerprints`):** `confirm proposal` → `people_writer.add_person_alias(workspace_root, <data.person_id verbatim>, <data.alias_name>)` (skip the alias write when the spelling already equals the record's canonical name), then a `same_as` tombstone per cluster entry (seq or fingerprint, as above), then `brain_proposals.resolve_proposal(..., "applied")`; `dismiss proposal` → `resolve_proposal(..., "declined")` **plus** a `not_relevant` tombstone per cluster entry (the underlying zombie proposals must die permanently — draining them is the point, D4a); `snooze proposal 7d` → standard bp snooze. NEVER dispatch `merge_person_into` from a person_link row — linking is an alias, not a record merge; **`kind: person_merge` (PID1 D4b — two EXISTING records look like one person; the row embeds `data.keep_id` + `data.duplicate_id` verbatim):** `merge person records` → `people_writer.merge_person_into(workspace_root, keep_id=<data.keep_id verbatim>, duplicate_id=<data.duplicate_id verbatim>, source_skill="apply-choices")` then `resolve_proposal(..., "applied")` — CONFIRM-ONLY FOREVER: `merge_person_into` has NO reverser and `person_merge` is never in `AUTO_ALLOWED`; no code path merges records without this click, and the ack says the merge cannot be undone; `proposal not relevant` → `resolve_proposal(..., "declined")` (60d cooldown — the suspect pair re-proposes only if still unresolved after it); `snooze proposal 7d` → standard bp snooze; `kind: org` → `confirm [type]` creates via `org_writer.create_org` (dedup-first through `find_existing_org`), `not relevant` → append `org_proposal_declined` naming the org; `kind: project` → `confirm [type]` routes to workspace-manager's new-project writer, `not relevant` → `chat_dismissal` (60d) — **STAFFCUT 2026-08-02: these two verbs now actually RIDE the row.** The org/project adapter shipped `action_tuples: []` hardcoded, so six live org rows rendered with no buttons while this dispatch entry sat here waiting for a click that could never happen; `brain_proposals._ORG_PROJECT_ACTIONS` puts them on the row. The handlers are unchanged — nothing new is written, and `confirm [type]`'s optional input is still the typed correction to the inferred name; `kind: dormancy` → `active` / `archive` / `snooze 14d`. **Dispatch on the TARGET KEY THE ROW ACTUALLY CARRIES, verbatim, and read it from the row rather than assuming which one it is (LIFECYCLE1 fix round):** a `bp_` row — every row the weekly lifecycle job writes — carries **`data.thread_id`**; a PRE-migration fossil row (wire id `dont_forget:<seq>`) carries **`data.target_id`**, which is whichever of thread / org / person its prose writer was looking at. Take `data.thread_id or data.target_id` and dispatch on that. Naming only one of them is how this broke: the handler said `target_id`, the bp writer emits `thread_id`, and a CEO tapping `archive` on a freshly-written dormancy row dispatched on nothing at all. The row is ON-DEMAND — it never appears on the staff meeting, only where the CEO asks about quiet projects (`stalled projects`; `brain_proposals.load_open_proposals(ws, "on-demand")` is the read). **The three handlers, in full — LIFECYCLE1 moved them here from the retired dont-forget orchestrator, unchanged; they are not re-derivable from anywhere else:** `active` → the project stays active; bp row → `brain_proposals.resolve_proposal(workspace_root, <bp_ id verbatim>, "declined", resolved_by=<user person_id>, source_skill="stalled-projects")` (tombstone + the shared 60d cooldown); a PRE-migration fossil row (legacy `dont_forget:` adapter id, no `bp_` prefix) instead writes `dont_forget_dormant_proposal_declined` (14-day cooldown, legacy semantics preserved). `snooze 14d` → `chat_dismissal` with `data.snooze_until` = now + 14 days and `data.target_id` = the row id verbatim. `archive` → skip the dormant step entirely and archive the project **through the canonical archive path — `thread_archive.archive_thread(workspace_root, <the row's own target key verbatim — `data.thread_id` on a bp_ row, `data.target_id` on a fossil one; take `data.thread_id or data.target_id`>, reason="archived from the dormancy review", source_skill="stalled-projects")`** — the SAME typed write workspace-manager's `archive [project]` runs. In one call it stamps `status`/`archived_at`/`archive_reason` via `thread_writer.update_thread`, appends ONE canonical `status_change` (`primary_thread_id` = the thread id, `data: {from_status, to_status: "archived", reason}`), and regenerates MASTER_TRACKER. Never hand-write the event here and never edit `entities.json`: this handler used to instruct a bare hand-built status change straight to archived, which produced a NON-canonical payload and NO record write at all — so the thread never actually reached `archived`, the tracker's next render brought the project back with its old status, and the archive the CEO clicked simply vanished (the third instance of that bug class; SPEC ARCHFIX fixed the other two). `res["status"] == "already_archived"` is an honest no-op — say so rather than claiming a fresh archive; a `ThreadArchiveError` means the row's `target_id` no longer resolves, so report it and never archive an adjacent project. THEN: bp row → `resolve_proposal(..., "applied")`; fossil row → the `status_change` that call wrote is its natural tombstone (60-day cooldown). The CEO's click IS the confirm — nothing on this rail archives anything on its own; `kind: schedule_add` → **RETIRED (STAFFCUT §3.6 + LIFECYCLE1 §7a) — no such row is emitted any more, and none is projected onto a named surface either (`brain_proposals.RETIRED_KINDS`).** 0 of 4 ever produced a registration, and registration only ever happened through the change-schedule / registration add path, which this dispatcher always refused to invoke. A pre-retirement row still sitting in a persisted widget dispatches as before: it is a pointer, `not relevant` → `chat_dismissal` (60d); **`kind: relationship_move` (WG1-B D-B4 — the SCHEDULED staff-meeting "THIS WEEK'S MOVES" row, built by `relationship_moves.moves_rows_from_candidates`; the row embeds `data.person_id` verbatim, `n` = `move:<person_id>`):** `nudge` → the standard compose-on-click chase path above, with the relationship-moves guard first: run `live_contact_check` for the person (the scheduled fire deliberately skipped it — connector-free render; THIS click is where it runs) — un-dormanted with no overdue-commitment carry → honest ack naming the recent touch, NO draft; otherwise compose the opener via the email-writer chain exactly as `relationship-moves/SKILL.md` specifies (voice corpus → Voice Block fallback, draft posture — never auto-sent); `snooze 3d` → `chat_dismissal` with `data.snooze_until` = now + 3 days, `data.target_id` = the row id verbatim AND `data.person_id` = the row's person_id verbatim (the person_id is what `relationship_moves._recently_excluded` keys on, so the snooze holds the person out of next week's compute too); `not relevant` → `chat_dismissal` (60d) carrying the same `target_id` + `person_id` shape. **The `FROM YOUR MEETINGS` section (CAPTUREFLOW §C, 2026-08-01; DONE1 2026-08-03) — rows carrying `confirm` / `already done` / `drop` / `not mine`, `n` = the commitment's `data.id` verbatim:** these are unconfirmed EXTRACTIONS from the needs-your-call queue, folded into the staff meeting by `needs_review_queue.staff_meeting_group_section`. They dispatch through the queue's OWN write path — the same one the on-demand surface uses — never through `resolve_proposal` and never through a per-surface fork. Collect every row of this section in the batch and dispatch by verb in ONE call each: `confirm` → **`needs_review_queue.confirm_items(workspace_root, <the ids>, source_skill="apply-choices", confirm_weak_ids=<only the wire ids the user named on their own>)`** — that call runs THE shared bulk-accept fence (`watch_gate.screen_bulk_accept`) internally, so a weak row is HELD and reported, never written; "Apply all", a range and a group/call phrase name nothing individually and pass an EMPTY set, and you never re-issue with the held ids on your own. `already done` → **`needs_review_queue.done_items(workspace_root, <the ids>, resolved_by=<user person_id>, source_skill="apply-choices", attested_ids=<ONLY the wire ids the user named on their own>)`** — DONE1: the row was a real capture AND the user already did it off-mail, so the call confirms it and closes it with `resolution="done"`, evidence = the user's own attestation and never a match, score or synthesized send. It runs THE SAME shared fence internally, plus a STRICTER caller-side bar: **every** id must appear in `attested_ids`, so "Apply all", a range and a call phrase — which name nothing individually and therefore pass an EMPTY set — write NOTHING and come back per item as `not_individually_named`. Never widen that set on your own and never silently downgrade a refused Done to a `confirm`: a confirm is a claim about what the workspace heard, this is a claim about what the CEO did. This verb is why a kept-off-mail promise no longer has to be answered with `drop` (a `dropped` closure is a DISMISSAL signal for that counterparty in `capture_gate`; a `done` closure is not). `drop` → `needs_review_queue.drop_items(workspace_root, <the ids>, resolved_by=<user person_id>)`. `not mine` → `needs_review_queue.not_mine_items(workspace_root, <the ids>, resolved_by=<user person_id>)` — the same closure with the honest reason; if the user NAMED the real owner, route to `commitment_state.reassign_commitment` instead. `handler_result` for each row is its own entry in the returned `results`. Ack in the queue's own plain counts (kept / already done / dropped / held), never event-type names, never ids. **Batch undo:** cache the batch's closed ids + dismissal seqs in the fire-marker — **and, since DONE1/UNCONFIRM1, its CONFIRMED ids and its `already done` ids alongside them**; a follow-up `undo` in the same chat reverses commitment closes via `commitment_state.reopen_commitment` and mutes via `mute_ledger.clear_dismissals` (the commitment-triage undo shape), **reverses queue confirms via `needs_review_queue.undo_confirm_items(workspace_root, <the confirmed ids>, restored_by=<user person_id>)` — the item returns to the needs-your-call queue carrying its ORIGINAL reason — and reverses `already done` answers via `needs_review_queue.undo_done_items(workspace_root, <the done ids>, restored_by=<user person_id>)`, which reopens the item AND returns it unconfirmed (a bare reopen would leave an item the user never confirmed sitting confirmed). Both route to `commitment_state.restore_review_flags`, the purpose-built un-confirm writer — NEVER `flag_duplicate_for_review` with an empty target, which writes a duplicate pair that does not exist into an append-only log. An un-confirm REFUSES an item somebody has since reassigned, parked, re-worded or re-closed (`touched_since_confirm`, and the refusal names what happened) — relay that and stop.** Brain-batch auto-applied changes reverse via `brain_undo.undo_batch` (additive `brain_change_undone` markers — never edit or delete prior events). Ack in plain change language ("Confirmed 2, declined 1 — the deal moved to negotiating."), never event-type names.)

**BINDING ROWS (BACKFILL2 2026-09-02) — the project-binding review on the needs-your-call source, ONE handler.** Wire id starts **`bind:`** (`bind:<seq>:<snapshot>` — the propose snapshot the row was derived from rides the id; you never parse or rebuild it). These rows come from `backfill_widget.render_backfill_page` (`src: "needs-your-call"`, needs-your-call Step 5) and carry exactly three verbs: `bind` / `not this project` / `skip`. **THERE IS NO PROPOSAL EVENT AND NO COMMITMENT BEHIND ONE OF THESE ROWS** — the row is a past substrate record that names a project it was never filed under, derived live by the R3 binding-backfill engine on every render — so `brain_proposals.resolve_proposal`, `needs_review_queue.confirm_items` and `commitment_state.*` are all WRONG here and will refuse or write against the wrong thing. Collect EVERY `bind:` tuple in the batch and dispatch them in **ONE call**:

```python
from backfill_widget import apply_choices
from primary_user import resolve_primary_user
result = apply_choices(WORKSPACE_ROOT, <the batch's bind: tuples, verbatim>,
                       applied_by=resolve_primary_user(WORKSPACE_ROOT) or "",
                       source_skill="needs-your-call")
```

Inside that call: `bind` rows become accepted seqs and `not this project` rows rejected seqs of ONE `backfill_bindings.apply` — one `reclassification` per bind through the event gate, ONE `bkf_` batch, every accept and reject recorded with its evidence tier in the `binding_backfill_run` receipt (the precision dataset); `skip` writes the standard one-day `chat_dismissal` targeting the wire id and adjudicates nothing; any other verb on a `bind:` row is refused per row and writes nothing (the no-auto fence — a binding row never becomes a bind by default, and "Apply all" applies the taps, not the page). The engine refuses the whole call when the substrate moved past the rows' snapshot (`stale_snapshot`) or when the batch mixes two renders; an empty `applied_by` refuses every bind/reject (an unattributed adjudication is not a precision record) while snoozes still land. `handler_result` for each tuple is its own entry in the returned `results` (`status` ∈ applied / recorded / dismissed / unchanged / refused / stale_snapshot). Ack with `result["summary"]` verbatim — it names the counts, the READY change when one happened, and the undo. **Undo:** cache `result["batch_id"]` in the fire-marker; a follow-up `undo` → `backfill_widget.undo_sitting(WORKSPACE_ROOT, <batch_id>, undone_by=<user person_id>, source_skill="needs-your-call")` (the standing `bkf_` reverser + one gauge rebuild); a bare `undo` in a later chat finds the same batch via `brain_undo.recent_auto_batches`. Never dispatch a `bind:` id anywhere else, and never let a non-`bind:` tuple reach this call.

**PERSON-CANDIDATE ROWS (SPEC PERSONLOOP1 2026-08-19) — the recurring unresolved names, on THREE surfaces, ONE handler.** Wire id starts **`pcand:`**; the row carries `data.name_key`, `data.candidate_name`, `data.observed_spellings`, `data.org_id`. It reaches the dispatcher from the needs-your-call queue (`src: "needs-your-call"`, section **PEOPLE THE GRAPH KEEPS MISSING**), from the staff-meeting card (`src: "cr-brain"`, same section, built by `person_candidates.candidate_section`), and from the End of Day's confirm block (`src` per that source's entry, `block: "person_candidate"`, payload off `end_of_day.resolve_choice`). **THERE IS NO PROPOSAL EVENT BEHIND ONE OF THESE ROWS** — a candidate is derived live from the pending queue by `person_candidates.derive_candidates` on every render, so `brain_proposals.resolve_proposal` and `confirm_flow.build_person_proposal_resolved_event` are both WRONG here and will refuse or write a tombstone for a proposal that does not exist. Every one of the three verbs dispatches through **ONE call** and nothing else:

```python
from person_candidates import resolve_candidate, ack_line
res = resolve_candidate(
    WORKSPACE,
    name_key=<the row's data.name_key verbatim>,
    action=<the verb verbatim: "add person" | "same as [existing]" | "not a person">,
    canonical_name=<the user's typed input when they corrected the spelling, else the row's data.candidate_name>,
    observed_spellings=<the row's data.observed_spellings verbatim>,
    org_id=<the row's data.org_id or None>,
    same_as_person_id=<'same as [existing]' ONLY: the person id the typed name resolved to>,
    answered_by=<user person_id>, source_skill="apply-choices")
```

- **`add person`** creates the record through `people_writer.create_person` with the canonical name and EVERY observed spelling saved as an alias (`aliases.json` keys on the RAW string, so the spelling that was failing is exactly the string written — a transcript misspelling resolves forever after), then fires `needs_review_queue.confirm_satisfied_reasons` so the rows the new record just released are durably cleared. `res["n_cleared"]` is that count and it goes in the ack: *"Added them — 9 waiting captures cleared."* A `needs_confirm` return means the name collides ambiguously with an existing record (Bug #19) — render the Step 3a disambiguation widget and write nothing; `already_on_file` means somebody added them between the render and the tap, which is an honest no-op on the create (the drain still ran).
- **`same as [existing]`** needs the person id, so resolve the typed name through the standard entity path FIRST and **ask when it is ambiguous — never guess**. Then the one call above; it writes the alias through `people_writer.add_person_alias` (both resolution surfaces) and drains.
- **`not a person`** appends ONE `person_candidate_suppressed` row, per name and per org, and writes nothing else. **It suppresses the PROPOSAL and only the proposal.** If the user's phrasing asks for more than that — "stop tracking anything from them" — say plainly that this answer stops the suggestion and the captures keep coming, and do not reach for a capture-side mute; there isn't one on this rail and there must not be. `already_suppressed` is an honest no-op.
- **NEVER call `create_person` / `add_person_alias` / `append_event` directly from this row.** The one call is the chokepoint: it is where the propose-only refusal lives (an absent or unrecognised `action` raises rather than defaulting), and it is what pairs every entity write with its drain. A hand-rolled write returns no result dict, so `handler_result` carries no `status` and the receipt reads `error` for a write that landed — the APPLYAUDIT1 class.
- **`handler_result` is `res`**, verbatim. Ack from `person_candidates.ack_line(res, name=<the name>)` — counts and plain words, never an event type, never a `pcand:` id.
- **Undo:** there is nothing to reverse on the derivation side (it wrote nothing). A wrongly-created person is corrected by the ordinary people-crm path (`merge person records` when it duplicates an existing record); a wrongly-suppressed name is re-offered by the user saying so — the suppression ledger is append-only like every other tombstone and no reverser is registered.
...
**One `src`, more than one emitter (SPEC_BOARD1, 2026-08-06).** `src: "commitment-triage"` no longer means "an in-chat widget" — it means the commitment-triage SURFACE, whichever page the user was looking at. The artifact **triage board** (`surface_drivers.py commitments --format artifact`, published as a private Artifact page and read from a phone between sessions) emits the identical tuples: `data.id` verbatim, a verb from the same per-kind sets, `src: "commitment-triage"`, and the same `apply choices: ` prefix — composed by a Copy button and PASTED by the user, because the board never writes. **There is nothing to add for it:** dispatch keys on id + verb and never on which surface emitted the click, so every handler below applies unchanged. Two consequences worth stating, because they are the ones that show up in chat: (a) a board is STALE by design — it does not refresh itself, so an already-closed id WILL arrive; `close_commitment` returns `already_resolved`, and the honest one-line ack ("that one was already closed") is the whole handling, never a hand-built tombstone; (b) `undo` still works, because the batch cache is in-session and the paste happens in-session.
...
| `push meeting [date]`, `push to [date]`, `schedule catchup [when]` | string (natural language: "monday at 2", "tomorrow afternoon", "2026-05-12"; for `push to [date]` ALSO a bare number of days — "5", "5 days") | For `push to [date]` run `commitment_state.parse_later_when(<input>, <now_iso>, workspace_path=workspace_root)` first (t3 FB-3: days-from-today and ISO dates resolve deterministically; `<now_iso>` is whatever clock stamp you hold — the parser anchors "today" on the workspace-local calendar day, so always pass the workspace, REVIEW_CUTC F-4); on `None`, and for the other verbs always, parse the natural-language datetime via the same chrono / date-parser logic the orchestrator uses for free-text user replies. The resolved ISO date then goes to `commitment_state.apply_later` — that writer does NO parsing of its own and raises on an unparseable `when_iso` rather than writing a mute that never expires. Empty input or unparseable string → surface item-level error in the consolidated ack ("couldn't read 'sometime soon' as a date — try again with a clearer time"). |
...
- **Button-rendered required-input verb (WG1-A D-A3, corrected by REVIEW_CUTC_2026-09-06 F-1).** On a small (≤4-option) row every verb renders as a button — and since the CUT-C fix round a REQUIRED-input verb's button (`theirs to [name]`, `reassign to [name]`, `same as [existing]`, `push to [date]`, `mark lost [reason]`, …) carries the same F-17 payload and inline field the dropdown option carries, so Apply holds until the value is typed and the tuple arrives WITH its `input`. A bare `{n, action}` for a required-input verb is therefore NOT a designed shape any more: it can only come from a page that was not relayed byte-exact from the transport. For `push to [date]` the Step 2 refusal applies verbatim (`commitment_state.later_missing_ack` — one line, nothing moved, no question). For the other required-input verbs the fallback below is unchanged tonight — do NOT error and do NOT drop the tuple; name exactly what is missing (`verb_taxonomy.required_input_thing(action_id)` — "a name", "a loss reason", …) and dispatch through the verb's normal handler once the value is given — and treat its firing as the symptom it is (a hand-built page), not as the button's contract:
        ...
        "items": [
            ...
            *[
            ...
            # Plus a final item for "different person, create new"
            {
                ...
                # PROVENANCE — this sentence is THIS SKILL's, not the user's.
                # `name` is normally user-authored and the renderer exempts it
                # from the internal-vocabulary scan on that basis; declaring it
                # composed keeps the template facing the full scan.
...
The intro-followup-check surface (`intro-broker`'s on-demand `check my intros` mode since LIFECYCLE1 — the retired Pulse chat carried it before, and a persisted Pulse widget can still dispatch these verbs) emits items with three domain-specific resolution verbs in its action set: `landed`, `didnt land`, `snooze 14d`. The renderer accepts these as canonical, but apply-time needs to write the right lifecycle event for each.
...
- `<n> snooze 14d` → schedule a re-emit. Append an `intro_followup_check` event (NOT a `chat_dismissal`):
  ...
  ```
  The next `check my intros` fire re-checks `scheduled_for` against today's date and re-surfaces the item when due. This differs from a standard 24h `chat_dismissal` — the snooze is a deliberate "check back in two weeks", not a "not now".
...
- **`already done` is NOT in the list above and must never be added to it (DONE1).** It is a needs-your-call QUEUE verb, dispatched only by the `FROM YOUR MEETINGS` handler in Step 2 through `needs_review_queue.done_items`. Routing it here — to a bare `close_commitment` — is exactly why it is not spelled `resolved`: it would close a `pending_review` row that was never confirmed, skip the shared bulk-accept fence, and skip the per-item attestation bar, and `close_commitment` would not refuse it because `user_confirmed=True` satisfies the pending-review guard.
...
- `add to my plate` → commitment_state.create_personal_task(workspace_root,
  ...
  **Pass the originating row's REAL source_ref or none at all — never invent one.**
  This is a user-initiated capture: the writer stamps `user:my_plate` when there
  is no genuine ref, and it replaces a `granola:<anything-not-a-uuid>` ref with
  that same scheme rather than write a connector provenance no connector minted
  (FLOOR2 C2 — a live row carried `granola:past-meetings-2026-08-04`, which made
  a hand-typed task read as a meeting capture to every reader that resolves the
  scheme). `source_event_seq` is the provenance link that always works.
...
- `keep as one` — CLUSTER1 (2026-08-25), terminal. The cluster line's one tap, on the render-clustered surfaces only: the needs-your-call queue widget, the staff meeting's FROM YOUR MEETINGS fold, commitment-triage's open sections, and the backlog-sweep digest's "Looks handled" / "Gone quiet" lists. **NEVER from `held-review`** — that surface renders clusters display-only per its DD-4 fence and embeds no `folded_ids`; refuse the verb there in one line and write nothing. The row embeds everything the dispatch needs, verbatim (the CLOSEID2 "id" door): `data.id` = the cluster's SURVIVOR, `data.folded_ids` = the folded rows. Dispatch ONE call and nothing else: **`commitment_cluster.apply_cluster_merge(workspace_root, <data.id verbatim>, <data.folded_ids verbatim>, merged_by=<user person_id>, source_skill="apply-choices", source_ref="session:<the dispatch's session/receipt id>")`** — internally that is the EXISTING writers only (`commitment_state.clear_review_flags` on a pending survivor + one `commitment_state.supersede_commitment(user_confirmed=True)` per folded id, details in evidence), every write stamped into ONE `clu_` brain batch. Never call either writer directly from here, never re-derive the cluster at dispatch time, and never resolve ids yourself — the widget-embedded ids are the whole contract. Honor `already_resolved` per-row as an honest no-op. Ack with the returned `summary` verbatim (it names the undo), and cache the returned `batch_ref` in the fire-marker: a follow-up `undo` reverses the whole gesture via `brain_undo.undo_batch(workspace_root, <batch_ref>, undone_by=<user person_id>, source_skill="apply-choices")` — the folded rows reopen onto the flag tier and the survivor's confirm is taken back, one gesture both ways.
- `bind`, `not this project` — BACKFILL2 (2026-09-02), terminal. The project-binding review's two taps, on `bind:` rows of the needs-your-call source ONLY; dispatched as ONE `backfill_widget.apply_choices` call per batch (Step 2's BINDING ROWS entry — never per row, never through a commitment or proposal writer). Ack with the returned `summary` verbatim; the batch id it returns is the undo.
- `mine`, `theirs to [name]`, `merge`, `keep both`, `add person`, `same as [existing]`, `proposal not relevant` — W4b confirm-section verbs (v4.6.1), all terminal. Dispatched per `orchestrator-commitments.md` § "Confirm section actions": `mine` → `commitment_state.confirm_commitment_owner`; `theirs to [name]` → `commitment_state.reassign_commitment(..., confirmed=True)` after the standard name-resolve; `merge` → `commitment_state.supersede_commitment(..., user_confirmed=True)` (survivor = the row's flagged duplicate target); `keep both` → `commitment_state.clear_review_flags`; the three person verbs run their entity write (Step 3a create / `people_writer.add_person_alias` / nothing) and then append the proposal tombstone via `confirm_flow.build_person_proposal_resolved_event` + `event_gate.append_event` — the tombstone is what stops the proposal re-surfacing, so it is never skipped, even for `proposal not relevant`. Acks in plain English; never event-type names. Guardrail restated: none of these ever stage or send a chase email — confirmation changes state, chase happens on later fires against CONFIRMED items only.
  ...
  **Orphan-note re-route (MLK1 D2, 2026-07-21 — supersedes the v3.13.0 list capture):** when the widget's `+ Add context` field had text but no action button was selected on that item, the renderer synthesizes a fallback `{n, action: "add to my list", context: "<typed text>"}` choice (see `chat_output_renderer.py` `crApplyAll` orphan-note capture — the carrier deliberately keeps the legacy wire id so old and new widgets emit one shape). The list is retired, so the capture NEVER writes a `commitment_to_discuss`. Route every context-bearing tuple of this shape through the canonical helper:
  ...
  THREE outcomes, all honest, and the key of record is **`status`** (APPLYAUDIT1 — `outcome` still rides along as a deprecated alias carrying the same word, and is deleted next release; read `status`): `{"status": "noted", ...}` — the source item resolved to a person or thread, and the helper appended ONE `note` event there (`data.via: "orphan_note_capture"`, `data.summary` = the typed text; gated writer inside the call) — ack in plain English ("Held that as a note on [name]."). `{"status": "already_noted", ...}` — this exact note (same target, same source row, same text) was already on disk and NOTHING was written; the helper is idempotent on identity (DOGFIX1 2026-07-27 — a re-dispatched payload wrote four notes twice). Ack it as the no-op it is ("Already held that one on [name]." / fold it into a consolidated count), NEVER as a fresh capture — a doubled ack on an undoubled write is the same lie in the other direction. `{"status": "declined", "line": ...}` — nothing resolved and NOTHING was written; surface the helper's `line` verbatim in the ack ("I need a person or project to pin that to — say more and I'll hold it properly"). A context-free fragment saved nowhere is better than a junk drawer — never fall back to a list write, and never guess a target the helper didn't resolve.

  **`handler_result` for this re-route is the helper's return dict, unchanged (Step 5).** It used to return `outcome` and nothing else, which `apply_audit.derive_outcome` — which reads `status` — could not read at all: every one of the retired wire id's dispatches in the defect register audited "error", the successful captures included, and page_snapshot then kept re-offering rows whose note was already on disk. The three words are now classified OK / no-op / refusal respectively. Never hand-build an outcome for this row, and never "correct" a `declined` into an ok because the ack sounded fine.
...
Per `shared/scripts/cru_match.py` Path 1. Runs ONLY when a `send` action just fired successfully (`edit then send` counts; `draft` does NOT — drafts aren't fulfillment). Conservative auto-resolve only; borderline matches go to a `pending_review` queue the confirm surfaces render.

Skip entirely if any of:
- The send failed (no commitment can have been resolved by a failed send).
- Sender's `person_id` cannot be resolved from entities.json (resolve the user with `primary_user.resolve_primary_user` and pull THAT record's `person_id` — do not scan for `is_user: true` / `is_primary_user: true`, which per SPEC USERKEY1/USERKEY2 are only the seam's flag fallback, behind every pointer spelling it reads including the canonical `workspace.user_id`).
...
# allocates seq inside the writer lock. A peeked value is racy the moment a
# concurrent fire appends first.
for r in results:
    # v2.14.7+: HIGH-confidence → auto-resolve, MEDIUM → pending-review for
    # confirm queue to surface as a one-click confirm/skip item.
    if r['recommendation'] == 'auto_resolve':
        try:
            res = close_commitment(
                ...
                source_skill='apply-choices',
                # PROVMINT1 — the SENT MESSAGE is what closed this commitment,
                # and its artifact key is already built above for the layer-1
                # fence. Forward it: an artifact pointer beats the surface
                # receipt the writer would otherwise mint, and building a good
                # ref for MATCHING and then dropping it at the CLOSE is the
                # class the 2026-08-17 walk found on three separate rails.
                source_ref=primary_artifact_key(provider, sent_message_id),
            )
        ...
        to_append.append(build_pending_review_event(
            ...
            # SPEC TITLEMINT1 — the matched commitment's own name, from this
            # result row. The builder REFUSES an empty title, so a forgotten
            # argument raises here instead of writing a subject-less row.
            title=r['title'],
...
Examples (good):
- *"Done — 4 of 5 applied. Rio added under Summit Company. Aspen logged for review. Item 4 needed a clearer date so it's still open."*
