...
- **`ghost_later_add`** (commitments / relationship-moves / commitment-triage / staff-meeting not registered) → EXPECTED, say nothing. A RETIRED task (`schedule_config.RETIRED_TASKS` — `pulse` since LIFECYCLE1) is the same silence for a different reason: it is gone, not pending, so never say it is missing and never propose adding it — the R3 proposal step owns that nudge.
