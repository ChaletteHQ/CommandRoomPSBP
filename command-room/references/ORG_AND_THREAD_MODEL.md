...
- **Never rank, sort, or compute staleness from it.** Recency derives from EVENTS at read time via `shared/scripts/thread_activity.py::derive_thread_activity()` (top-level `primary_thread_id` + `related_thread_ids` + legacy spellings, `classification_confidence >= 0.40`). Recency consumers also honor user-approved reclassifications (RECL1, 2026-07): every adopted surface passes `honor_reclassifications=True`, so a correction moves activity WITH the event fleet-wide — the kwarg is the greppable adoption ledger. Consumers: `stall_detector` / stalled-projects, `lifecycle_pass`, deal rot, the master tracker, list-active, the workspace map, DCC, org history, `go [name]` recency ranking, and every view that already follows the `computed_last_activity` rule in VIEW_GENERATION.md.
...
- `active → dormant` — auto-transition, run by the weekly `lifecycle` maintenance job (`shared/scripts/lifecycle_pass.py`; the retired Pulse chat carried it until LIFECYCLE1). No activity in 30+ days → it ASKS, as a one-key dormancy question on the on-demand `stalled projects` surface. 60+ days quiet AND that question left unanswered until its 30-day life ran out → auto-flips to dormant. An answered question stops it; so does a NEW question still standing; and the silence stops authorising the flip 30 days after the question lapsed — past that the job asks again rather than acting on stale silence.
- `dormant → active` — auto-revive. `go [project]` + adding a session note flips back to active. Or explicit `revive [project]`. The weekly `lifecycle` job also revives on its own when activity lands in the last 7 days — activity that has actually HAPPENED (a future-dated calendar entry is not a revival) and that does not contradict a status the CEO set by hand.
- `dormant → archived` — auto-flip after 180 days dormant without revival, written by the weekly `lifecycle` job through `thread_archive.archive_thread` (the canonical archive path, so the record, the `archived_at` stamp and the `status_change` all land as they do for a human archive). Capped per fire; the remainder stays due.
- `archived → active` — explicit `revive [project]`, or the weekly `lifecycle` job when real activity lands on the thread again (its own auto-archive yields to new evidence, including evidence captured after the fact; an archive a HUMAN made is only undone by activity that post-dates it). Either way the archive stamps are cleared through `thread_archive.clear_archive_stamps` — an active project must never keep an archive date.
