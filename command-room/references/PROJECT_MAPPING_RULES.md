...
Examples:
- 3 attendees: M, aria@example.com, bowie@example.com → all-non-M share `category.example.com` → matches `org_005.domains` → primary thread `project_002`
...
If multiple rules fire with **conflicting** answers (e.g., title alias points to Acme Phase 1 but attendee domain points to Category Company), DO NOT pick. Stage to the unrouted folder with a plain-English banner at the top of the file. The banner does NOT name internal rules (Rule A/B/C/D, "deterministic", "domain consensus" etc.); it describes the conflict in plain English the user can act on:

```markdown
> **Couldn't pick a project — two reasonable matches.**
>
> Best guess from the title: **Acme Phase 1**
> Best guess from who's on the invite: **Category Company**
>
...
>   - **Category Company / Q3 strategy** (last touched 4 days ago, has "Q3" in recent notes)
>   - **Aspen Hardware / Quarterly review** (matches "review")
>
> If this is a new client or contact, add them in chat ("add Q3 to Category Company") and the next run will route automatically.
```
...
- **Multi-org meetings** (e.g., Q3 review with Acme + Category attendees) — Rule B fails (mixed domains), Rule A may fire if title contains the right alias. If Rule A doesn't fire, item lands in `_unrouted/` with a banner showing the mixed-domain detection. M routes manually.
